...
**What this document changes.** The owner has clarified that Himma is the **final production marketplace** — not a prototype, not a disposable MVP, not a customer-app-only build. This document rebaselines the roadmap accordingly. It does not restart anything: every approved milestone, decision, and screen stands. It re-anchors them inside a platform-wide plan in which the customer app is one of three frontends, and the backend is one of seven workstreams.

**Amendment A2 (2026-08-05, documentation-only consistency and correctness pass).** Preserves A1 and corrects only: workstream count updated to **seven** after W7's addition (§intro, §3); C5 corrected — customer web is **pending the §18.8 owner decision**, not presumed future (§17); §10's requirements formally numbered §10.1–§10.13 with wrong cross-references fixed (§5, §6.3); CapacityHold creation rewritten as a single atomic transaction with no committed capacity-claimed-without-active-hold intermediate (§6.4); a capacity-floor invariant added — capacity can never drop below confirmed + active holds outside a controlled disruption workflow (§6.3, §10.1); the §8.5 bulk-import partial-commit contradiction resolved (explicit new validated subset batch, itself atomic); a binding transactional-outbox/consumer-inbox requirement and recoverable payment/booking saga definition added (§10.13); Auditor access restricted to scoped, masked, purpose-bound, logged/watermarked review (§7); and the G6 pre-launch crash-free target raised to ≥ 99.8 % with zero known critical checkout crashes and a ≥ 99.9 % post-launch target (§15). Superseded A1 wording recorded in C17 (§17).
...
Seven workstreams (W7 added by Amendment A1). Each gets its own planning documents on the docs/20–22 pattern (spec → owner decisions → staged commits → closeout) as it activates.
...
| **Session** (occurrence) | program id, schedule id?, branch id, start/end datetime, capacity, booked count, session state (§6.3), per-session eligibility override? | Capacity truth lives here, guarded by DB constraints (§10.1). |
...
| **CapacityHold** (InventoryReservation) | id, session/campweek ref, booking-draft ref, account id, quantity (1 now), state (§6.4), created at, expires at (TTL), consumed-by booking id? | The server-side reservation that makes paying safe: payment may only be captured against an `active` hold, and confirmation consumes the hold and the capacity **in one transaction** (§6.4, §10.1). |
...
**6.3 Session**
`scheduled → open → full (auto, capacity) → open (on cancellation)` · `open|full → closed (registration cutoff)` · `scheduled|open|full → cancelled_by_provider` · `closed → completed (after end time)`. Capacity changes must transition state atomically with the booked count (§10.1). **Capacity-floor invariant: a session's capacity may never be reduced below `confirmed bookings + active holds` by any direct edit — provider-portal and admin capacity edits are rejected below that floor.** Reducing below existing commitments is only possible through the **controlled disruption workflow**: a provider/admin-initiated partial-cancellation flow that selects affected confirmed bookings (selection policy owner-decidable; default latest-confirmed-first), transitions them via §6.5 `cancelled_by_provider` with §6.7 refund handling, notifies affected customers (§12.7), releases their capacity, and records the full action — reason, actor, affected bookings — in the audit trail. The invariant is enforced at the database layer alongside §10.1's overselling constraint.

**6.4 Capacity hold (InventoryReservation)**
States: `active → consumed (atomic with booking confirmation — same DB transaction writes hold=consumed, booking=confirmed, booked_count final)` · `active → expired (TTL lapse, capacity released) | released (customer abandons / payment terminally fails, capacity released)`. **Creation is atomic: one database transaction claims the capacity and inserts the hold already in `active` state (TTL started), or does neither.** There is no `created` pre-state and no committed intermediate in which capacity has been claimed without a usable active hold existing — a hold row either exists as `active` with its capacity claimed, or the transaction rolled back and capacity is untouched. Rules: the hold transaction runs when the customer commits to pay (checkout confirmation begins), **before** any charge; **payment capture is only permitted against an `active` hold** (a lapsed hold aborts the attempt pre-charge, or triggers an automatic same-amount reversal in the §12.4 race window — the customer is never left charged without capacity); hold creation fails fast with `sessionFull` when no capacity remains (§12.3); expiry/release restore capacity atomically; holds are short-TTL (minutes, tuned per gateway flow) and never presented to customers as "reserved" beyond the truthful in-checkout window. This is the mechanism that closes the pay-without-capacity gap: capacity is guaranteed for exactly the interval money can move, and both are settled in one transaction.
...
| **Auditor** | read-only, **data-domain-scoped** | audit review within explicitly granted domains (financial, verification, access, support, …) with a recorded **purpose/reason per engagement** and an **expiry** on the grant where the engagement is time-bound; **PII masked by default** (names, contacts, child data, payment references shown redacted); unmasking is a **separately authorized, per-record or per-case grant**, individually logged; exports are permitted within scope only, logged, and **watermarked** with the auditor identity, scope, and timestamp | any mutation; unscoped browsing; unmasked PII without the separate grant; unlogged or unwatermarked exports |
...
4. **Error reports** — per-row errors with line numbers and reasons, downloadable. **There is no partial commit of a failed batch**: after reviewing the error report, the user may explicitly form a **new batch from the validated subset** (a deliberate, previewed action), and that new batch then commits atomically like any other — rule 3's atomicity is never weakened, and no rows ever apply as a side effect of a failed batch.
...
13. **Event integrity and orchestration (binding):**
    - **Transactional outbox:** every domain event (booking confirmed, session cancelled, refund approved, …) is written to an outbox table **in the same PostgreSQL transaction as the business state change that caused it** — a committed state change with no event, or an event with no state change, is structurally impossible. A relay publishes outbox rows to the queue with at-least-once delivery.
    - **Consumer inbox:** every consumer records processed event ids in an inbox (or equivalent idempotency store) inside its own processing transaction, so at-least-once delivery yields exactly-once effects.
    - **Payment/booking orchestration is a recoverable saga, not a distributed transaction.** No design may pretend to atomicity across PostgreSQL and the payment gateway — that transaction does not exist. The checkout saga's steps (quote → hold → intent → gateway attempt → confirm) each commit locally with outbox events; every step has a defined **compensating transaction** (release hold, expire intent, post same-amount reversal — §6.4/§6.6) and a recovery point, so a crash or timeout at any step converges via the saga's resumption or compensation plus the reconciliation job (§6.6 GatewayEvent convergence, `ReconciliationEvent`) — never via manual data surgery and never by leaving a customer charged without a confirmed booking or capacity.
...
- **G6 Native quality:** §14 pass complete on every shipped screen; crash-free sessions **≥ 99.8 %** in pre-launch tracks with **zero known critical crashes anywhere in the checkout/payment path**; store review passed on both stores. Post-launch operational target: **≥ 99.9 %** crash-free sessions, tracked from day one (G8 monitoring) with regression alerting.
...
| C5 | `docs/03` §§5–9 "Future product surface" | Provider portal, admin portal, customer web framed as unscheduled futures. | **Superseded:** W2/W3 are planned workstreams with sequence positions. **Customer web is pending the §18.8 owner decision — neither presumed future nor unscheduled.** (Himma for Business remains genuinely future/unscheduled.) |
...
| C17 | **docs/23 as amended by A1 (pre-A2)** | Contained: stale "six workstreams" wording after W7's addition · C5 still calling customer web "genuinely future/unscheduled" despite §18.8 · ambiguous/wrong §10.2 citations · a two-state hold creation (`created → active`) permitting a committed capacity-claimed-without-active-hold intermediate · no capacity-floor invariant against reductions below confirmed + active holds · §8.5 error-report wording ("apply valid rows") contradicting batch atomicity · no transactional-outbox/inbox requirement and implicit reliance on cross-system atomicity for payment/booking flows · an unscoped, unmasked read-everything Auditor · a 99.5 % pre-launch crash-free target with no checkout-crash condition. | **Superseded in place by Amendment A2** (§intro, §3, §6.3–6.4, §7, §8.5, §10.1, §10.13, §15 G6, §17 C5). Correction trail continues; no A1 position survives unamended where A2 speaks. |
