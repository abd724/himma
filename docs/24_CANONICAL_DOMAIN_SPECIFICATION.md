...
**Amendment A3 (2026-08-07, owner ruling at S4-1 review — documentation-only, applied in place):** gender eligibility uses the DISTINCT five-value vocabulary **`women | men | girls | boys | mixed`**, superseding the first draft's `men | ladies | mixed`. `women` is the canonical stored code; customer UI may render it as "Ladies only" where appropriate — presentation wording never determines database semantics. `girls`/`boys` are explicit provider-declared eligibility required for catalogue discovery/search (a girls-only activity is representable directly) and are **never inferred from `minimum_age`/`maximum_age`**, which remain independent constraints. The §2.5.2 rule is unchanged in substance: adults are never gender-filtered automatically; "Ladies only" remains an explicit customer filter, now over `gender_eligibility = 'women'`. The mock's three-value `GenderEligibility` type remains a conforming presentation-era contract that maps `ladies → women` at graduation (§12.1). Corrected in §2.5 and §12.1; the database correction is backend migration `0009_gender_eligibility_correction`.

**Amendment A4 (2026-08-26, S6-1 — the canonical amendment owned by the OWNER-APPROVED docs/35 S6-0 plan, closure `3b7d770`; applied with backend migration `0017_commercial_target_and_entitlement_foundation`):** the commercial spine is generalized over **exactly one commercial target** per payment — a capacity **Booking** (unchanged) or a unit-less **EntitlementPurchase** (D-RI-1/D-S6-1: unit-less package/pass sales are never forced into the capacity Booking model and never fake a capacity unit). Canonical additions/corrections:

1. **PriceQuote commercial shapes (§4.1):** every quote carries `commercial_shape ∈ capacityPurchase | entitlementAcquisition | entitlementReservation` — capacityPurchase = exactly one capacity unit (today's certified shape, unchanged); entitlementAcquisition = ZERO units, bound to the immutable fulfillment revision it sells; entitlementReservation = one unit + zero total + the exact owned Entitlement (S6-3 operational scope; structural from `0017`). Cross-trail crossing is structurally impossible (quote↔unit composite FKs on holds; the purchase shape trigger; the five-column entitlement ownership FK).
2. **`membership` price kind (§2.6; resolves §14.B7's membership half per D-S6-3):** COMMERCIAL vocabulary only. Fulfillment semantics (finite/unlimited, uses, validity, reservation-required, walk-in) live on the new **PriceOptionFulfillmentRevision** (§2.6) — immutable revisioned terms: a provider edit supersedes and inserts the next revision; sold quotes/entitlements reference their frozen revision, so later edits never rewrite purchased terms. `weekly` remains owner-open.
3. **EntitlementPurchase + Entitlement (§3.4):** the parallel unit-less commercial anchor and its append-only participant-owned grant; **PackageEntitlement is SUPERSEDED** (dropped in `0017` — never authoritative, provably empty; entitlement truth lives on `Entitlement` with append-only S6-2 attendance as the future usage authority, never a mutable used-counter).
4. **PaymentIntent commercial target (§4.1):** `booking_id`/`hold_ref` nullable + `purchase_id`, exactly-one-target CHECK, Booking⇔hold shape CHECK, one live / one succeeded intent per target; D-W5-7 economics unchanged with the org binding resolved through the ACTUAL target. Trusted success dispatches OUTSIDE the seams: Booking → `confirmPaidBooking` (unchanged); EntitlementPurchase → `confirmPaidEntitlementPurchase` (equally HTTP-unreachable). Zero-price acquisition NEVER creates payment state (Purchase + Entitlement insert atomically as `confirmed`; no observable zero-price `pending_payment`).
5. **EntitlementPurchase lifecycle:** `pending_payment → confirmed | payment_failed | expired | compensated`; `payment_failed → pending_payment | expired | compensated`; terminals frozen; `expires_at` is checkout-abandonment metadata only (a Purchase owns no inventory). Validity starts at acquisition CONFIRMATION (D-S6-4); first-use activation does not exist in V1.

**Amendment A5 (2026-08-26, S6-2 — applied with backend migration `0018_redemption_attendance_reservation`):** the check-in/attendance authority ships: **RedemptionCredential** (the one-time short-lived customer-generated check-in authority — opaque ≥128-bit token digest as the canonical secret, org-scoped 8-digit numeric alias digest, TTL 10 min, exactly one live credential per fulfillment target, supersede-on-regenerate, three unambiguous target forms: plain session Booking · reserved entitlement use · walk-in entitlement), the **redefined append-only AttendanceRecord** (§3.5 — created only by atomic provider redemption; `credential_id` UNIQUE as the structural single-use backstop; finite usage derives from counting consuming rows; one attendance per session-backed Booking occurrence; no-show consumes nothing; the first draft's mutable marking/corrections are deferred future semantics), the **EntitlementReservation structural subtype** (S6-3 operates it), and the **`attendance.manage` capability activation** (owner · org manager · branch manager scoped · front desk scoped · coach ONLY with `session.instructor_staff_id` assignment; `roster.view` stays RESERVED). Provider validation is preview (pure read, org-scoped, brute-force-bounded by durable PostgreSQL attempt windows) + atomic redeem; camp/cohort check-in stays FAIL-CLOSED pending S6-3 occurrence identity (no fake occurrences) *(superseded by Amendment A6 — canonical per-occurrence check-in shipped with `0020`)*. S6-3 (reservation creation/`availableToReserve`/calendar/fulfillment reads) remains RESERVED/future *(shipped at S6-3, `5858fce`)*.

**Amendment A6 (2026-08-31, S6-3 Final Correction — the owner-approved D-S6-5/D-S6-6 correction plan `2b04c81`; applied with backend migration `0020_membership_booking_and_multi_occurrence_attendance`):**

1. **Membership reservation Booking compatibility (D-S6-5; §3.4):** `Booking.option_kind` admits **`membership`** — the reservation Booking a membership-kind `entitlementReservation` quote confirms into. The commercial trails are unchanged: membership ACQUISITION remains `entitlementAcquisition → EntitlementPurchase → Entitlement` (never an ordinary Booking — acquisition quotes are unit-less/unholdable); ordinary `capacityPurchase` still structurally excludes `membership` (the A4 quote-shape CHECK); a reservation quote is zero-total and can never gain a PaymentIntent; the ONLY creator of a membership-kind Booking is `confirmEntitlementReservation`. Commission remains once-at-sale.
2. **Canonical multi-occurrence identity (D-S6-6; §2.3, §3.5):** a scheduled attendance occurrence is identified by **`occurrence_date + occurrence_start_time`** (Asia/Dubai civil identity). **Session:** `session_id` remains the canonical occurrence — unchanged. **CampWeek:** every civil date in `[start_date, end_date]` is exactly one running occurrence at the camp's single daily window (V1 has no internal closure/exception days — DEFERRED with the owner; every included date runs). **EnrolmentCohort:** occurrences derive from the CURRENT canonical meeting model (active cohort⇄schedule associations → active RecurringSchedule ∩ windows, weekday match, minus exception dates); **date-only identity is FORBIDDEN** — two schedules may legally meet on one civil date at different times.
3. **RedemptionCredential occurrence binding (§3.5):** booking-target credentials carry EITHER their `session_id` (Session) XOR the occurrence pair (CampWeek/Cohort — both columns, never half-populated); walk-ins carry neither. The pair is written by the SERVER from the canonical schedule at issuance and is **frozen for the credential lifetime** — later schedule edits never retarget an issued credential; future issuance follows the then-current canonical schedule (no cohort schedule-revision snapshotting exists). Exactly one LIVE credential per Booking **per occurrence** (adjacent occurrences' ±60-minute windows may overlap, so different-occurrence credentials coexist); issuance requires EXPLICIT occurrence selection for multi-occurrence Bookings — "nearest to now" inference never exists — and enforces the −60/+60 window and the not-already-attended check on the canonical occurrence instant.
4. **AttendanceRecord occurrence binding (§3.5):** attendance copies (and is trigger-proven equal to) the consumed credential's frozen pair — the SCHEDULED occurrence, permanently distinct from `occurred_at` (the actual redemption instant; a cross-midnight redemption stays bound to its scheduled occurrence date/time). Uniqueness: one attendance per session-backed Booking occurrence (unchanged) and one per `(booking, occurrence_date, occurrence_start_time)` for multi-occurrence Bookings — the same camp day or cohort meeting can never attend twice while every other valid occurrence stays independently attendable; `credential_id` UNIQUE remains the exactly-once backstop.
5. **Calendar convergence (§2.3 narrative):** the derived customer Calendar represents a CampWeek Booking as one event per canonical DAILY occurrence (keys `booking:<id>:<date>:<HH:MM>`, converging with attendance identity; the overall span rides as presentation metadata, never replacing occurrence truth); Cohort expansion and credential issuance share ONE canonical derivation authority; exception dates yield no Calendar event and no credential; event keys never change because an attendance was recorded.
6. **Provider authority:** owner/org-manager/branch-manager/front-desk check-in extends to camp/cohort occurrences under their certified scoping; **coach stays FAIL-CLOSED for CampWeek/Cohort** (no occurrence-level assignment proof exists — CampWeek has no instructor field and `RecurringSchedule.instructor_staff_id` is schedule-level, optional, and mutable; coach occurrence assignment is DEFERRED).
...
Structure unchanged from the approved model (docs/05 §7, `Eligibility` in `src/types/domain.ts`): `minimum_age?`, `maximum_age?` (null = open-ended), `all_ages`, `gender_eligibility` (**Amendment A3:** `women` \| `men` \| `girls` \| `boys` \| `mixed` — `women` is the canonical stored code; the mock's `ladies` maps to it at graduation), `skill_level?`, `notes?`. Attached to Program; overridable per Session. Server rules, binding everywhere:

1. Child suitability is **purely age-based**, computed against the unit's start date.
2. Adults are **never** gender-filtered automatically; "Ladies only" is an explicit customer filter/badge over `gender_eligibility = 'women'` (Amendment A3 — the badge wording is presentation, the code is canonical). `girls`/`boys` are explicit provider-declared values, never derived from the age range.
...
- **Catalogue price (Amendment A2; A4):** price kinds live on **ProgramPriceOption** rows (§2.2) — one row per commercial shape, ≥ 1 per Program; the launch kind subset is `dropIn`, `monthly`, `term`, `camp`, `package`, `free`, plus **`membership` (Amendment A4, D-S6-3 — commercial vocabulary ONLY)**, with amounts in **integer fils** (entitlement kinds `package`/`membership` may carry a genuine zero amount — a free introductory product's real price). `weekly` remains a model-ready extension pending the owner (docs/09 §21.14, §14.B7). The mock's unused `freeTrial` price kind is dropped — trials derive from Offers (HANDOFF; §12.2). No single authoritative `Program.price` exists; customer-facing `from`-price displays are derived at read time from active options (docs/28 §14 boundary).
- **PriceOptionFulfillmentRevision (Amendment A4):** the IMMUTABLE fulfillment-terms authority for one entitlement-producing option (`package`/`membership`): **id**, **price_option_id** (same-program composite ref), revision_no, usage_kind (`finite` \| `unlimited`), uses_total? (finite membership; `package` totals derive from the certified `sessions_count`), validity_kind (`daysFromConfirmation` \| `fixedEndDate` \| `none` — finite-only), validity_days?/validity_end_date?, reservation_required, walk_in_allowed (≥ 1 true), branch_id? (null = all branches), state (`active` \| `superseded`), plus the schedule-term snapshot child (weekday/time VALUES copied at revision creation for schedule-bound membership products — never a reference to mutable RecurringSchedule rows). At most one ACTIVE revision per option; rows are frozen — a provider edit supersedes and inserts; sold quotes/entitlements reference their frozen revision. **Fulfillment semantics never live in a price kind.**
...
| **EntitlementPurchase (Amendment A4)** | **id**, **account_id** (purchaser), **participant_id** (beneficiary, account-composite), organization_id, program_id, price_option_id, offer_id?, **quote_ref** (unique — one acquisition quote → one Purchase, ever), **fulfillment_revision_ref** (immutable terms), state (A4 §5 lifecycle), reference_code (customer-facing, non-enumerable), expires_at (checkout-abandonment metadata ONLY — never inventory), confirmed_at?, created_at, version | The parallel UNIT-LESS commercial anchor (D-RI-1/D-S6-1): a package/membership sale has no capacity unit and gets none — no fake Session, no hold. Paid acquisition rests `pending_payment` beside its PaymentIntent; zero-price acquisition inserts atomically AS `confirmed`. |
| **Entitlement (Amendment A4)** | **id**, **purchase_ref** (unique — exactly one grant per confirmed Purchase; 7-column identity composite), account_id, **participant_id**, organization_id, program_id, price_option_id, **fulfillment_revision_ref**, usage_kind (`finite` \| `unlimited` — EXPLICIT; unlimited stores NO count), uses_total? (finite), **valid_from** (= acquisition confirmation, D-S6-4), valid_until?, reservation_required, walk_in_allowed, branch_id?, created_at | **Append-only** participant-owned grant: created complete at confirmation, never edited; later provider edits never rewrite it. Belongs to ONE participant (cross-account/participant substitution structurally impossible; no transferable/family package in V1). Finite usage truth = uses_total − the future S6-2 append-only attendance count — never a mutable used-counter. |
| ~~PackageEntitlement~~ | **SUPERSEDED (Amendment A4, D-S6-2)** — dropped by migration `0017`: never authoritative, provably empty, booking-anchored (the exact coupling D-RI-1 forbids) with a mutable used-counter design. Entitlement truth lives on **Entitlement** above. | |

### 3.5 Attendance

| Entity | Fields | Rules |
|---|---|---|
| **AttendanceRecord (redefined by Amendment A5; occurrence binding by Amendment A6)** | **id**, organization_id, branch_id?, account_id, **participant_id**, entitlement_id?, booking_id?, session_id? (canonical occurrence when session-scheduled), **occurrence_date? + occurrence_start_time?** (A6 — the frozen canonical camp/cohort occurrence, copied from the consumed credential; paired, never half-populated; NEVER derived from `occurred_at`), **credential_id** (unique — structural single-use), **validated_by** (staff membership), occurred_at (the actual redemption instant — distinct from the scheduled occurrence), source (`numericCode` \| `qr`) | **APPEND-ONLY attendance truth** — the ONLY V1 usage-consumption authority: created exclusively by the atomic provider redemption of a live RedemptionCredential; finite entitlement usage derives from counting consuming rows (never a mutable used-counter); no-show consumes nothing. The first draft's mutable `expected/no_show/excused` marking and windowed corrections are DEFERRED future semantics (a correction, when it ever arrives, is an append-only reversal record — never an update). Attendance feeds future payout accrual and the verified-review gate unchanged. |
| **RedemptionCredential (Amendment A5; occurrence binding by Amendment A6)** | **id**, token_digest (unique — sha256 of the ≥128-bit opaque canonical token; QR-ready), alias_digest (org-scoped digest of the 8-digit numeric alias), exactly one target (entitlement_id \| booking_id), session_id? XOR **occurrence_date? + occurrence_start_time?** (A6 — Session Bookings carry their session; CampWeek/Cohort Bookings carry the frozen canonical occurrence pair, server-derived at issuance and immutable for the credential lifetime; walk-ins carry neither), account_id, **participant_id**, organization_id, branch_id?, issued_at, **expires_at** (TTL 10 min default, server config), state (`live` \| `used` \| `superseded` \| `expired`), used_at?, redeemed_by? | The customer-generated one-time short-lived check-in authority (docs/35 §9). Raw secrets exist only in the issuance response — never at rest, never in audit/outbox/idempotency stores. Exactly one LIVE credential per fulfillment target; regeneration supersedes; issuing/expiry/supersession consume NOTHING; `expires_at <= now()` is unusable regardless of lifecycle cleanup (no sweeper needed). |
| **EntitlementReservation (Amendment A5 — structure; S6-3 operates)** | **booking_id** (subtype PK), **entitlement_id**, account_id, participant_id, organization_id, program_id | One finite-entitlement unit temporarily allocated to ONE future capacity Booking (ownership/lineage composite-FK-pinned). Commitment/fulfilled/released/no-show status is DERIVED from Booking state + occurrence time + attendance — no mutable status column. Operational creation is the S6-3 `confirmEntitlementReservation` authority; no route/command exists before it. |
...
| Entity | Fields | Rules |
|---|---|---|
| **PriceQuote** | **id**, program_id, option_kind, **commercial_shape (Amendment A4: `capacityPurchase` \| `entitlementAcquisition` \| `entitlementReservation`)**, unit ref (exactly one for capacity/reservation shapes; NONE for acquisition), fulfillment_revision_ref? (acquisition only), entitlement_ref? (reservation only — five-column ownership composite), participant_id, lines (kind `base` \| `discount` \| `fee` \| `tax` \| `credit`; label; amount_fils), total_fils (must equal the line sum — §6.3), currency, price_kind (`oneOff` \| `cadence` \| `free`), tax_treatment, **expires_at**, created_at | The server money truth (§3.2) — exactly ONE commercial fulfillment shape per quote (A4). Until the owner's VAT/fee decisions exist, quotes emit `base` lines only and `tax_treatment = 'notConfigured'` — the structure is complete, the config is empty (docs/09 §22.2–5). |
| **PaymentIntent** | **id**, **exactly one commercial target (Amendment A4): booking_id (+hold_ref) \| purchase_id**, quote_ref, amount_fils, currency (`AED`), state (§5.8), **idempotency_key** (unique), expires_at, created_at, version | One per checkout confirmation attempt-series; a retried confirmation request rejoins the same intent via its idempotency key. One live / one succeeded intent per target; the economics snapshot's organization is the TARGET's organization (D-W5-7, target-neutral). |
...
| `Eligibility` | Shape unchanged (§2.5); **Amendment A3:** `genderEligibility` value set widens to `women\|men\|girls\|boys\|mixed` — mock `ladies` recodes to canonical `women` at graduation; "Ladies only" survives as presentation wording | extended (value vocabulary) |
...
| B7 | `membership`/`weekly` price kinds vs the `monthly` mapping. **Partially resolved (Amendment A4, D-S6-3):** `membership` shipped additively as COMMERCIAL vocabulary with fulfillment semantics on PriceOptionFulfillmentRevision; `weekly` stays open | Price-model widening | docs/09 §21.14 |
