...
| Entity | Fields | Rules |
|---|---|---|
| **Program** | **id**, **organization_id**, branch_ids (≥ 1, same org), **activity_type_id** (→ category derived), title (en, ar?), description (en, ar?), media_refs, setting (`indoor` \| `outdoor`), eligibility (embedded, §2.5), price_model (§2.6), policy_ref (cancellation policy version, §2.7), listing_state (§5.3), sensitive_fields_version, created_at, version | The bookable offering. Only `published` listings are searchable/bookable. Edits to admin-designated **sensitive fields** (price, eligibility, safety copy) re-enter review; other edits hot-publish (docs/23 §6.2). Bilingual columns exist from day one (docs/23 §3.1.1) and stay nullable until the W7 content workflow is decided (§14.B12). |
...
### 2.3 Schedules, sessions, camp weeks

| Entity | Fields | Rules |
|---|---|---|
| **RecurringSchedule** | **id**, **program_id**, weekday/time pattern, timezone (`Asia/Dubai`), effective_start, effective_end?, exceptions (dates), registration_cutoff_rule, instructor_id?, enrolment_capacity? (§2.4), state (`active` \| `ended`), version | The authority that replaces the mock `scheduleLabel` derivation (docs/23 §5). Session generation from schedules is an idempotent job (§7 note); generated sessions may then be individually managed. |
...
Holds, bookings, and the capacity invariants operate on a **capacity unit**: `(unit_kind, unit_id)` where `unit_kind ∈ {session, camp_week, enrolment_pool}`.

- `session` — one dated occurrence (drop-in, free session, trial session).
- `camp_week` — one camp week.
- `enrolment_pool` — the seat pool of a RecurringSchedule for monthly/term enrolments (`enrolment_capacity`, with its own booked/held counts on the schedule row). A recurring class has finite places exactly like a session does; enrolment bookings claim a pool seat through the same hold machinery. **Owner confirmation requested** (§14.A2): docs/23 §5 defines holds against session/camp-week refs only; this document extends the identical mechanism to enrolment seats as the only non-inventing way to make monthly/term bookings capacity-safe. Until confirmed, W4 must not implement enrolment booking.
...
- **PriceModel** (catalogue price): the seven implemented kinds (`dropIn`, `monthly`, `term`, `camp`, `package`, `free`, `freeTrial`) are the launch subset; amounts move from AED numbers to **integer fils**. `membership` and `weekly` are model-ready extensions pending the owner (docs/09 §21.14, §14.B7). The unused `freeTrial` price kind is dropped in the production model — trials derive from Offers (HANDOFF; §12.2).
...
| **Enrolment** | **booking_id** (subtype), schedule_id, billing_anchor_date, cadence (`monthly` \| `term`), renewal_policy (**owner-open**, docs/09 §6 — column exists, no value semantics until §14.B5) | Subtype rows for monthly/term bookings. No auto-renewal behavior may be implemented before the owner decision. |
...
| **Refund** | **id**, originating transaction ref, booking_id, amount_fils, destination (`original_method` \| `marketplace_credit`), reason, state (§5.9), **initiated_by**, **approved_by** (must differ — §6.5), resulting refund-transaction ref, created_at | Dual control is a database invariant, not a UI convention. |
...
### 5.4 Session (docs/23 §6.3) — CampWeek mirrors this machine
...
| 7.5 | **Cancel a booking (customer)** | Evaluate policy snapshot (pure) → insert CancellationCase with the evaluated outcome → CAS booking `confirmed → cancelled_by_customer` → lock unit row, decrement `booked_count` (future units only) → if `refund_due`: insert Refund `initiated` → outbox `booking.cancelled`, `refund.initiated?` | The refund gateway execution (own saga step after approval). docs/23 §6.7 names support/ops as refund initiators; whether a policy-evaluated `refund_due` may be **system-initiated** (still finance-approved — dual control holds either way) is proposed here and flagged for owner confirmation with the §14.B6 template decision |
| 7.6 | **Cancel a session (provider/admin disruption)** | CAS session → `cancelled_by_provider` → for each affected confirmed booking (selection recorded): CAS → `cancelled_by_provider`, decrement `booked_count`, insert CancellationCase + Refund `initiated` per policy → release affected active holds → insert SessionDisruption `executed` with the full action record → outbox `session.cancelled` + per-booking `booking.cancelled` | Notification sends (consume events); refund execution |
...
| 8.6 **Hold-lapse race** | If capture evidence arrives after the hold expired (§7.4b aborts): post an automatic **same-amount `reversal` PaymentTransaction**, intent → `failed`, booking → `expired`; customer told honestly | This is the mechanism guaranteeing "never charged without capacity" (docs/23 §6.4, §12.4) |
...
1. **Server authority:** pricing (quotes), eligibility, availability, and every §5 status are computed server-side; responses may include display-ready labels, but structured data is the truth and clients never do money arithmetic or permission logic (docs/08 §14).
...
| `PriceModel` (7 kinds, AED numbers) | PriceModel in fils (§2.6); `freeTrial` kind dropped (trials are Offers); `membership`/`weekly` pending owner | extended + conflict (fils; `monthly`-as-membership mapping pending §14.B7) |
...
| `BookingService.getBookingOptions` | Booking-options endpoint (server-computed options, availability, skip rule, household eligibility, preselection) | Unchanged shape; availability derives from §5.4 states |
...
| 6 | **Sessions & availability** | RecurringSchedule, Session/CampWeek generation (idempotent job), §5.4 machine, registration cutoffs (server, Asia/Dubai), capacity fields + §6.2 constraints, availability read APIs feeding details/booking-options contracts, capacity-floor edits, §3.6 disruption workflow skeleton | Schedule → session generation idempotent (re-runs create nothing new); details/booking screens read real availability behind unchanged contracts; cutoff transitions server-computed; capacity edit below floor rejected at API **and** DB layers | Scheduler edits another org's schedule → denied; front-desk edits capacity → denied; coach reads another instructor's roster → denied | Property test: random interleaved capacity edits + bookings never violate §6.2 (DB-verified after run); concurrent generation jobs → no duplicate sessions (natural-key constraint) |
| 7 | **Capacity holds & bookings** | CapacityHold + §5.5, PriceQuote (base-lines-only config), Booking + §5.6, free-booking confirmation (§7.3), hold expiry job (§7.2), cancellation case skeleton (§7.5, refund execution stubbed pending W5), outbox events, customer Bookings read APIs | End-to-end **free** booking confirmable against real capacity (paid confirmation waits for W5 + §19 lift); hold TTL expiry releases capacity; every §7.1–7.5 boundary implemented as specified single transactions with outbox rows; booking reference codes non-enumerable | Customer books another account's participant → denied; customer books an unpublished program → denied; provider staff creates customer bookings → denied; customer cancels another's booking → denied | **Final-seat gate (early run):** N ≥ 50 concurrent holds on 1 seat → exactly 1 active hold, N−1 typed `sessionFull`, zero constraint violations (docs/23 §11.1 rehearsal); expiry job racing confirmation → exactly one of consumed/expired wins via CAS; idempotent confirmation replay returns the stored outcome without double-increment |
...
| A2 | **Enrolment capacity model** — confirm the `enrolment_pool` capacity unit (§2.4 default: schedule-level seat pool in the identical hold machinery) or direct an alternative | Slice 6–7 for monthly/term programs (drop-in/camp paths are unblocked) | §2.4 |
...
| B6 | **Cancellation/refund policy templates** (windows, proportions, credit incentives, auto-approval rules — incl. whether policy-evaluated `refund_due` refunds may be system-initiated per §7.5, versus support/ops-initiated as docs/23 §6.7 words it) | Template content, §7.5 refund automation, provider agreement | docs/09 §7, docs/23 §18.14 |
...
*Prepared as the docs/23 §16 P0 canonical-model deliverable. Approval of this document is docs/23 §18 Batch-1 adjacent (item A1 here) and enables W4 slice planning per §13; it starts no implementation, lifts no prohibition, and changes no approved screen.*
