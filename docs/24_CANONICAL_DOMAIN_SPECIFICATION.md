...
| Entity | Fields | Rules |
|---|---|---|
| **AttendanceRecord (redefined by Amendment A5)** | **id**, organization_id, branch_id?, account_id, **participant_id**, entitlement_id?, booking_id?, session_id? (canonical occurrence when scheduled), **credential_id** (unique — structural single-use), **validated_by** (staff membership), occurred_at, source (`numericCode` \| `qr`) | **APPEND-ONLY attendance truth** — the ONLY V1 usage-consumption authority: created exclusively by the atomic provider redemption of a live RedemptionCredential; finite entitlement usage derives from counting consuming rows (never a mutable used-counter); no-show consumes nothing. The first draft's mutable `expected/no_show/excused` marking and windowed corrections are DEFERRED future semantics (a correction, when it ever arrives, is an append-only reversal record — never an update). Attendance feeds future payout accrual and the verified-review gate unchanged. |
| **RedemptionCredential (Amendment A5)** | **id**, token_digest (unique — sha256 of the ≥128-bit opaque canonical token; QR-ready), alias_digest (org-scoped digest of the 8-digit numeric alias), exactly one target (entitlement_id \| booking_id + its session occurrence), account_id, **participant_id**, organization_id, branch_id?, issued_at, **expires_at** (TTL 10 min default, server config), state (`live` \| `used` \| `superseded` \| `expired`), used_at?, redeemed_by? | The customer-generated one-time short-lived check-in authority (docs/35 §9). Raw secrets exist only in the issuance response — never at rest, never in audit/outbox/idempotency stores. Exactly one LIVE credential per fulfillment target; regeneration supersedes; issuing/expiry/supersession consume NOTHING; `expires_at <= now()` is unusable regardless of lifecycle cleanup (no sweeper needed). |
