...
**Status: DECISION PACKAGE — this commit, awaiting the owner's provider/region approval. ANALYSIS/PLANNING ONLY: no cloud account, resource, IaC, pipeline, domain, credential, Cognito configuration, or product behavior was created or changed; `productionChargingPossible` stays the literal `false`; migration head stays `0023_job_run_and_maintenance_authority`.**
