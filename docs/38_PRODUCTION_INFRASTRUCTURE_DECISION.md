...
**Status: OWNER-APPROVED (2026-09-07, recorded at the W6-4A bookkeeping commit): AWS · primary region `me-central-1` · separate Staging and Production workload accounts under AWS Organizations (no Himma workloads in the management account) · Cognito User Pools, ECS/Fargate and RDS PostgreSQL Multi-AZ/PITR confirmed available in the region · RDS PostgreSQL Multi-AZ is the production database direction · the closed W6 application architecture is unchanged · docs/36 IN-01 CLOSED by this ruling · W6-4 infrastructure work is UNBLOCKED (W6-4A = repository-side foundation; W6-4B = staging apply/certification). Originally: DECISION PACKAGE, awaiting the owner's provider/region approval. ANALYSIS/PLANNING ONLY: no cloud account, resource, IaC, pipeline, domain, credential, Cognito configuration, or product behavior was created or changed; `productionChargingPossible` stays the literal `false`; migration head stays `0023_job_run_and_maintenance_authority`.** **W6-4A (AWS Infrastructure & Deployment Foundation) OWNER-APPROVED / CLOSED at `cba2bd8` (2026-09-08) — chain `0393601` (decision) → `6f53800` (implementation) → `06e8c6e` (container certification) → `cba2bd8` (provisioning/CI concurrency). Frozen at closure: migration head `0023_job_run_and_maintenance_authority` · backend Jest 1418/1418 across 124 suites · db:verify 23/0 · production container harness 35/35 · Trivy HIGH/CRITICAL 0 · Customer App 549/549 · Playwright 15/15 · Provider Portal 815/815 + contract 77/77 · Admin 138/138 + contract 15/15 · docs/36 79 open · 59 P0 · 20 P1 · `productionChargingPossible` = literal `false`. The container artifact and the CI/provisioning concurrency boundary are CERTIFIED. W6-4B (Deploy and Certify Himma STAGING) is AUTHORIZED — staging only; production untouched; no LIVE payments.**
...
## W6-4A · AWS Infrastructure & Deployment Foundation — IMPLEMENTATION RECORD (2026-09-07 at `6f53800` — OWNER-APPROVED / CLOSED at `cba2bd8` (2026-09-08); repository-side ONLY: NO AWS resource applied, NO credentials, NO production apply, NO live payments, NO product change; `productionChargingPossible` stays the literal `false`; migration head stays `0023`)

**Authority:** owner authorization W6-4A (2026-09-07) on the OWNER-APPROVED §13/§14 ruling (IN-01 CLOSED at `0393601`). **Environment facts at implementation:** this machine has no container runtime (no Docker/Podman/Colima), no Terraform (installed `hashicorp/tap/terraform` 1.16.0 as local dev tooling for `fmt`/`validate` only), and no AWS credentials/accounts (`~/.aws` absent, no `AWS_*` env). Therefore: everything repository-side was built and validated; the container image build/run and any `terraform apply` were NOT executed here (recorded honestly; CI runs the image build on the first push; staging apply is W6-4B). **[Superseded 2026-09-08 by the container-certification correction record below: the image was built and every container proof executed locally on a real OCI runtime; only `terraform apply`/AWS resources remain unexecuted.]**

**Reconciliation (before editing, at `0393601`):** the W6 runtime is unchanged and code-complete (`start:api`/`start:worker`/`start:maintenance`, `db:migrate`/`db:verify`/`db:provision-roles`); `createPool` passed no TLS option and no pool bounds (docs/38 §2.6 debt); every `pg.Client` construction (`migrations.ts`, `provision-runtime-roles.ts`) built its own options; `loadConfig` accepted only a full `DATABASE_URL`; no Dockerfile, `.dockerignore`, compose file, CI workflow, IaC, or `eas.json` existed; the frontends take public build-time config only (`EXPO_PUBLIC_*`, `VITE_*` — no secrets); the portals need a shared registrable domain with the API for credentialed cookies; the payment seam composes absent-or-TEST only; Cognito is reached directly by the app and mediated by the backend; the evidence driver is SigV4 S3.

**Shipped:**
- **PostgreSQL TLS/pool hardening (the only application-code change, infrastructure-required):** typed `DatabaseSslConfig` (`disable` | `verify-full` + CA bundle — no weaker mode exists) and `DatabasePoolConfig` on `DatabaseConfig`; `loadConfig` parses `DATABASE_SSL_MODE`/`DATABASE_SSL_CA_FILE` (CA read and PEM-validated at load), `DB_POOL_MAX`/`DB_CONNECT_TIMEOUT_MS`/`DB_IDLE_TIMEOUT_MS`/`DB_STATEMENT_TIMEOUT_MS` (bounded), and the `DATABASE_PASSWORD` companion for a password-less URL (managed-secret injection; RDS-managed passwords are not URL-safe); **production refuses**: a missing mode, `disable` for any non-loopback host, a `verify-full` without a readable PEM CA, a URL carrying `sslmode`/`ssl`/`sslrootcert`, URL password + `DATABASE_PASSWORD` together. The ONLY plain-TCP production path is an explicit `DATABASE_SSL_MODE=disable` against a LOOPBACK host — the certification harness (docs/37 §36); the W6-1…W6-3 spawned-process suites now declare it explicitly. One helper (`src/db/connection-options.ts`) derives every `pg` client/pool option (`ssl: { ca, rejectUnauthorized: true }` = chain + hostname verification; pool `max`/timeouts; session `statement_timeout` per role: api 15 s · worker 60 s · maintenance 300 s · migrate none) and is used by the pool, the migration runner, and role provisioning — no path can drift. Runtime-proven (`test/db-tls.test.ts`) against a REAL TLS-enabled PostgreSQL started for the test with a self-signed CA: migrations/provisioning/pool over TLS (`pg_stat_ssl` true, TLS 1.2/1.3), a wrong CA refused, the non-TLS server refused (never a fallback), and a **spawned production `start:api` booting READY over verify-full as `himma_api`** with `statement_timeout = 15s`, refusing to start without the contract, no secret in output.
- **`backend/Dockerfile` + `.dockerignore`** — the ONE artifact: `node:24-bookworm-slim`, multi-stage (`npm ci --omit=dev` from the lockfile; `tsx` is a runtime dependency by W6-1 design), non-root `himma` (10001), node as PID 1 (`ENTRYPOINT node tsx`; SIGTERM reaches the W6 handlers), no `.env`, no tests/fixtures/git/dev executables in the context, no secret in any ARG/ENV, OCI labels (revision/date), the public AWS RDS global CA bundle baked at `/etc/himma/certs/rds-global-bundle.pem` with `DATABASE_SSL_CA_FILE` preset; commands select api/worker/maintenance/migrate/verify/provision-roles. **Not built at `6f53800`; BUILT AND CERTIFIED in the 2026-09-08 correction (record below — two defects found and fixed: CA directory modes, tsx CLI entrypoint)**; CI builds it, asserts non-root/no-.env/no-dev-scripts/CA-present, proves the production process refuses to start without `DATABASE_URL`, and scans it (Trivy).
- **`infra/local/`** — the production-like container harness (docs/37 §36): TLS PostgreSQL (harness CA, server cert CN/SAN `postgres` — verify-full hostname checking genuinely exercised over a NON-loopback host) → migrate → verify → provision-roles → api ×2 → worker ×2 → explicit-profile maintenance; `harness.sh` generates certs/passwords into gitignored files and checks migration head, TLS on every application backend, DB identities per replica, readiness/liveness/dev-404/forged-401, scheduler runs without overlap across two workers, `retention.all` as `himma_maintenance_runner`, in-container privilege refusals, SIGTERM exit 0 with the other replica ready. **Authored and syntax-checked at `6f53800`; EXECUTED AND PASSING in the 2026-09-08 correction (record below).** The equivalent proofs also run natively in `db-tls.test.ts` and the W6-1…W6-3 spawned-process suites.
- **Terraform (AWS provider ~> 6, Terraform ≥ 1.10; `terraform fmt -check` and `validate` GREEN for the stack and the bootstrap):** one root stack `infra/terraform/stacks/himma` applied per WORKLOAD account with `envs/<env>/terraform.tfvars` + `backend.hcl` (separate S3 state per account, encrypted, versioned, S3-native locking); provider pinned to `me-central-1` and `allowed_account_ids = [expected_account_id]`. Modules: **network** (VPC, 2 AZ, public/app/database subnets, database subnets with NO default route, NAT 0/1/2 by env, S3 gateway + optional ECR/Secrets/Logs interface endpoints, security groups: internet→ALB 80/443 only, ALB→api 8080 only, api/worker/jobs→RDS 5432 + HTTPS egress, worker/jobs no ingress) · **database** (RDS PostgreSQL 16, Multi-AZ by env, gp3 encrypted, `rds.force_ssl=1`, 7–35-day backups/PITR, deletion protection, final snapshot, master password **RDS-managed** — no plaintext in state) · **secrets** (containers only: `db/api`, `db/worker`, `db/maintenance`, `app/peppers`, `stripe/test`, `evidence/s3`, `auth/bootstrap`; no value ever written by Terraform) · **registry** (ECR immutable tags, scan on push) · **compute** (ECS Fargate cluster; four task definitions from the one image — api service behind the ALB with circuit-breaker auto-rollback, worker service with NO load balancer, maintenance run-task under `himma_maintenance_runner`, migrate run-task under the RDS master secret; **one execution role per invocation reading ONLY its own secrets**; task roles with no AWS permissions except the API's scoped evidence policy; `readonlyRootFilesystem`, non-root user; `stopTimeout` ≥ drain budget; EventBridge Scheduler runs `retention.all` daily — the only scheduled maintenance command; `payments_mode` validated to `disabled|test`) · **ingress** (ALB, ACM DNS-validated, HTTP→HTTPS redirect, TLS 1.3 policy, target health `/internal/ready`, deregistration = drain, Route 53 alias; `root_domain`/hostname parameterized, zone looked up or created — ownership never assumed) · **static_site** ×3 (portal, admin, customer web/universal-link/bounce surface: private S3 + CloudFront OAC, HTTPS-only, immutable `/assets/*` caching, `index.html` revalidated, SPA fallback for the portals, security headers, TLS-only bucket policy) · **evidence_bucket** (private, block-public, versioned, SSE, TLS-only, prefix-scoped put/get/head policy, NO delete, NO lifecycle — VE-05 unruled; VE-02 content safety untouched) · **cognito** (one pool per environment, TOTP optional at pool level, three public clients — mobile/portal/admin — with exactly `USER_PASSWORD_AUTH` + `REFRESH_TOKEN_AUTH`, token revocation, no hosted UI, no social providers) · **observability** (log group with bounded retention, metric filters on the four W6 alert codes + startup refusals, alarms: ALB 5xx, unhealthy targets ≥ 2 min, RDS CPU/storage/connections, ECS running tasks below desired; SNS topic; optional email) · **github_oidc** (OIDC provider; `build` role = ECR push from any ref of the repository, staging account only; `deploy` role trusted ONLY from the GitHub environment named after the account, scoped to register/run the four families, update the two services, read job logs, deploy the static buckets). `bootstrap/` creates the state bucket per account with local state. `envs/*/terraform.tfvars.example` carry non-secret placeholders (account id, domain, repository) — filled copies are gitignored.
- **Scripts:** `tf-preflight.sh` (prints only account/region/environment; refuses mismatch; production refused without `HIMMA_ALLOW_PRODUCTION_APPLY=yes`), `bootstrap-db-secrets.sh` (mints the three runtime passwords straight into Secrets Manager — never printed), `ecs-deploy.sh` (migrate run-task → verify → register/rollout → readiness/smoke; rollback = previous SHA; never `db:down`).
- **GitHub Actions:** `ci.yml` (backend tsc/lint/Jest on a PostgreSQL service + `db:migrate`/`db:verify` on a fresh DB + `npm audit --audit-level=high`; customer app tsc/lint/Jest; portal/admin tsc/lint/Jest/contract on PostgreSQL; production image build + invariant checks + fail-closed startup probe + Trivy; Terraform fmt/validate) and `deploy.yml` (OIDC only; **deploy-DISABLED by construction until the account/role variables exist**; staging from `main`; production only by `workflow_dispatch` + the `production` GitHub environment's required reviewers; no secret values as inputs; no live mode exists to select).
- **Security locks (`test/infra-locks.test.ts`, 12 tests):** Dockerfile (multi-stage, Node 24, non-root, node PID 1, no secret ARG/ENV, no `.env`, dev executables excluded, RDS CA baked), harness (production/verify-full/four identities/no dev or deterministic/only api exposes a port), RDS private+encrypted+TLS-forced+backed-up+RDS-managed password, database subnets unrouted, only ALB open to 0.0.0.0/0 (80/443), worker/jobs no ingress, API the only ALB-attached service, maintenance/migrate not services, only `retention.all` scheduled, per-task secret scoping (api/worker/maintenance never see the master secret or each other's), only the API task role touches evidence, no IAM users/access keys, `payments_mode ∈ {disabled,test}` + no `live`/`sk_live` anywhere, buckets block-public/encrypted/versioned/TLS-only with no evidence lifecycle or delete, region pinned + account pinned + remote locked state, Cognito public clients with the certified flows only, workflows OIDC-only with no static keys, production never auto-deployed, no `db:down` in any pipeline path, application boundaries re-pinned (`productionChargingPossible` literal false, no `rejectUnauthorized: false`).
- **Docs:** `infra/README.md` (layout, account topology, network/egress, 12-step bootstrap, state, secret ownership matrix, DB identities, deploy/rollback/migrations, DR outline + restore rehearsal, observability, harness, cost); docs/36 gate rows advanced (no closure); HANDOFF.

**Bounded deviations / notes:** (1) `DATABASE_PASSWORD` companion added to the config contract (infrastructure-required: RDS-managed master passwords and per-role runtime secrets are injected as values, not URLs); (2) the loopback plain-TCP exception exists ONLY for the certification harness and is refused for every non-loopback host — RDS endpoints can never use it; (3) `tsx` runtime execution is preserved in the image (W6-1 decision; no transpile step introduced); (4) the RDS CA bundle is fetched at image build time (public trust anchor; sha256 printed per build — pin by digest at the first real build); (5) the evidence secret is an access-key pair today because the certified driver signs with static SigV4 credentials — moving it to task-role credentials is a recorded follow-up inside the closed W3 driver's option surface; (6) Terraform 1.16.0 was installed locally as dev tooling; no other tool was added.

**docs/36 gate impact (this commit — evidence-driven, NO closures):** IN-04, IN-05, IN-06, IN-10, IN-11, IN-12, IN-13, SE-01, SE-03, ID-01, VE-01 → **ADVANCED (IaC/code-complete; still CONFIGURATION/EXTERNAL/SMOKE-gated)**. Tallies unchanged: **open 79 · P0 59 · P1 20** · IMPLEMENTATION-REQUIRED 26 · EXTERNAL-DEPENDENCY 19 · OWNER-DECISION 13 · CONFIGURATION-REQUIRED 9 · PRODUCTION-SMOKE-REQUIRED 8 · BLOCKED 4. Closed since LR-0: 10 (IN-01/02/07/09, OP-01/02/03/04/05/07).

**Verification (this commit):** backend Jest **1412/1412 across 123 suites** (was 1392×121; +db-tls, +infra-locks, TLS/pool config tests) · backend tsc/ESLint clean · `db:verify` **23/0** · `terraform fmt -check` + `validate` GREEN (stack + bootstrap) · workflows/compose YAML parsed · shell scripts `bash -n` clean · Customer App Jest **549/549 across 41** + tsc/lint · Customer Playwright **15/15** · Provider Portal **815/815 across 63** + contract **77/77 across 7** · Admin **138/138 across 15** + contract **15/15** · `productionChargingPossible` literal `false` (re-pinned). **NOT executed here:** Docker image build/run, the compose harness, any `terraform plan/apply`, any AWS resource — no container runtime and no AWS credentials exist on this machine. **[2026-09-08 correction: the image build/run and the compose harness ARE now executed — record below; `terraform plan/apply` and AWS resources remain unexecuted.]**

**W6-4B external prerequisites (owner/company, minimum):** (1) two AWS workload accounts under the Organization (staging, production) with an engineering IAM identity (SSO/role — never root) in each; (2) confirmation of the company domain and, if it exists, its Route 53 hosted zone (or permission to create one and delegate NS); (3) the GitHub repository location (`owner/repo`) and creation of the `staging`/`production` GitHub environments (production with required reviewers); (4) an operator to run the §4 bootstrap with `aws` CLI access from a workstation (never CI) — it writes account ids/domain into the gitignored var files and passwords straight into Secrets Manager; (5) later, per gate: Stripe TEST credentials (PA-02), peppers (generated by the operator), an alert email/destination (OP-08), Apple/Google credentials (ID-05…07). Production apply stays refused until staging is certified and the owner reviews.

## W6-4A · CONTAINER CERTIFICATION — OWNER-CORRECTION RECORD (2026-09-08 at `06e8c6e`; on top of `6f53800`; container artifact ACCEPTED by the owner; OWNER-APPROVED / CLOSED at `cba2bd8` (2026-09-08))

**Blocker (owner review of `6f53800`):** the production image and the local container harness were authored but never executed. **Resolution:** an OCI runtime was installed as ordinary engineering tooling (Colima 0.10.3 → Lima VM `vz`/virtiofs, Ubuntu 24.04 guest, Docker Engine 29.5.2, Docker CLI 29.8.0, Compose 5.5.1, Buildx 0.37.0; Trivy installed for the CI-equivalent scan), the exact CI build path was executed (`context: backend`, `file: backend/Dockerfile`, `HIMMA_GIT_SHA`/`HIMMA_BUILD_DATE` build args), and the harness was extended into a full certification transcript and executed to green. **No AWS credentials, no Terraform apply, no AWS resource, no product/payment/policy change.**

**Defects found by real execution (all fixed narrowly; none visible to native tests):**
1. **RDS CA directory not traversable by the non-root user.** BuildKit's `ADD --chmod=0444` applied 0444 to the directories it created; `/etc/himma/certs` was unreadable for uid 10001, so `DATABASE_SSL_CA_FILE` could never be read at runtime (the CI invariant step would have failed too). Fix: `chmod 0555 /etc/himma /etc/himma/certs` (bundle stays 0444).
2. **Entrypoint ran the application in a child process and needed a writable `/tmp`.** `node tsx/dist/cli.mjs` forks the program (PID 1 was the tsx CLI, the app its child) and creates an IPC socket under `/tmp/tsx-<uid>` — with the ECS read-only root filesystem (no tmpfs) the process crashed before the application loaded. Fix: `ENTRYPOINT ["node", "--import", "/app/node_modules/tsx/dist/loader.mjs"]` (in-process loader; the application IS PID 1, ppid 0) + `TSX_DISABLE_CACHE=1` (no compile-cache writes) → the image runs with **zero writable locations** (proven: `--read-only`, no tmpfs, `touch` refused on `/tmp`, `/app`, `/var/tmp`, `/`). The ECS `readonlyRootFilesystem = true` needs no tmpfs.
3. **Migration/verify/provisioning invocations crashed in the image (`MODULE_NOT_FOUND: dotenv`).** `scripts/cli-env.ts` imported `dotenv` (a devDependency) at module load. Fix: lazy `require('dotenv')` only when `NODE_ENV !== 'production'` and a `.env` exists — the production image resolves every shipped invocation from production dependencies alone (now asserted by the CI invariant step for all six invocations under `--read-only`).
4. **A live-shaped `STRIPE_SECRET_KEY` with `PAYMENTS_MODE=test` let the API START (payments composed as unavailable) instead of refusing the process.** Composition already refused the key (never reached a driver — docs/37 §33); the runtime contract now refuses startup outright in production for any non-TEST-mode key in any payments mode (`ConfigError`, names the variable, never the value; unit-tested). Live enablement remains the future PA-06 change, never configuration.
5. **CI scan gates would have failed on first push:** Trivy HIGH in `fast-uri` 3.1.5/4.1.2 (transitive of Fastify/ajv) and in the base image's bundled npm (`brace-expansion`, `ip-address`, `tar`); `npm audit --omit=dev --audit-level=high` reported the same `fast-uri` HIGH. Fix: lockfile-only semver bump `fast-uri` → 3.1.7 / 4.1.4 (`npm audit` gate → clean; two MODERATE Fastify advisories remain below the gate threshold and are recorded for the next dependency pass), and the runtime stage drops npm/npx/corepack/yarn (never invoked — ENTRYPOINT is node) → Trivy CRITICAL/HIGH = 0 (`--ignore-unfixed`, the CI policy).
6. **Base image pinned by digest** as the Dockerfile promised at the first real build: `node:24-bookworm-slim@sha256:ba849c60be29959425b8734d57b8b4b7d56f98edd9504c9af091d5281095a71e` (multi-arch index digest; Node 24.20.0, Debian 12.15). RDS global CA bundle sha256 at this build: `e5bb2084ccf45087bda1c9bffdea0eb15ee67f0b91646106e466714f9de3c7e3` (108 certificates).
7. **Harness script defects (macOS bash 3.2 + `pipefail`):** empty-array expansion under `set -u`; `grep -q`/`grep -m1` closing a `docker logs` pipe early failed the pipeline; a psql denial's non-zero exit failed the `| grep 'permission denied'` check. Fixed (logs captured to variables; helper returns output only).

**Certification transcript (`infra/local/harness.sh`, final run on this tree — HARNESS PASSED, 34 proofs; executing solely from the built image, no host source mounted; every backend container `read_only: true`, no tmpfs):**
- **Image:** `himma-backend:ci` image id `sha256:e16c2566d1b2d8e6a21fbe20db4cbf7b77fd89ba77f0444c720cfcee465e1100` (93 MB; the npm removal happens in a runtime-stage layer, so the base layer bytes remain — the files are absent from the final filesystem) · linux/arm64 (this machine; CI builds amd64 from the same digest-pinned index) · user `himma:himma` (uid/gid 10001) · Node v24.20.0 · entrypoint in-process loader · no `.env`/`.git`/tests/dev executables/README/config · no npm/yarn · env carries no secret-shaped name · full-filesystem search: 0 dotenv files, 0 `.git`, 0 AWS credential files, 0 live/test Stripe keys, 0 AWS access keys; the only "PRIVATE KEY"/password-URL text hits are library documentation/parsers (npm docs — now removed, `jose` key import, pg READMEs).
- **Fresh database from zero, from the image:** `db-migrate` (exit 0, 23 migrations applied) → `db-verify` (exit 0, "23 migration(s) applied, 0 pending. Schema verification passed.") → `db-provision-runtime-roles` (exit 0; created `himma_api`, `himma_worker`, `himma_maintenance_runner`; `himma_app`/`himma_maintenance` NOLOGIN present). Head `0023_job_run_and_maintenance_authority`.
- **API ×2:** both ready (`{"status":"ready"}`), both `himma_api` over TLSv1.3, dev identity 404, forged bearer 401, independent `x-request-id` per replica; session from the canonical pool inside the image: `login=himma_api statement_timeout=15s TimeZone=UTC ssl=true`.
- **Worker ×2:** both `himma_worker` over TLSv1.3; both log `loops:["outbox","scheduler"]` with the four scheduled jobs and the required head; `job_run` rows accumulate (16 in the window) with 0 overlapping runs per job; no published port; status listener answers 200 on loopback inside the task and is `ECONNREFUSED` from the network. Session: `statement_timeout=1min`.
- **Maintenance:** `retention.all` as `himma_maintenance_runner` → 5 `job_run` rows succeeded, exit 0, container gone, identity not resident afterwards; unknown job → exit 2 (usage); the api or worker credential under `RUNTIME_ROLE=maintenance` → refused (identity assertion). Session: `statement_timeout=5min`.
- **Migration authority:** api against an un-migrated database → running but `/internal/ready` 503 and **no DDL**; worker → startup refused (`pgmigrations`), no DDL; `db-migrate` under the `himma_api` credential → `permission denied`; two concurrent migration jobs on a fresh database → both exit 0, head reached once, 23 applied, 0 duplicates; re-running `db-migrate` at head → exit 0, applied set unchanged; every runtime identity denied `CREATE TABLE`, `ALTER TABLE`, and writes to `pgmigrations`; `himma_api` denied `DELETE`, `himma_worker` denied `SET ROLE himma_maintenance`.
- **TLS from inside the image (`verify-full`, harness CA, server SAN `postgres`):** positive one-shot `db-verify` over TLS passes; wrong CA (the baked RDS bundle) → refused (`self-signed certificate in certificate chain`); hostname mismatch (network alias `postgres-unlisted`) → refused (`Hostname/IP does not match certificate's altnames`); `DATABASE_SSL_MODE=disable` against the non-loopback host → refused; missing mode → refused; `?sslmode=` in the URL → refused. `rejectUnauthorized` is never false; no hostname bypass.
- **Payment invariants from inside the image:** `PAYMENTS_MODE=live` → refused; `PAYMENTS_MODE=deterministic` → refused; live-shaped `STRIPE_SECRET_KEY` (placeholder, not a credential) with `PAYMENTS_MODE=test` → refused at startup; `paymentCapabilityReport('production', {paymentsMode:'disabled'}).productionChargingPossible === false` evaluated through the real entrypoint.
- **Signals:** SIGTERM to a worker container → handler logs `"signal":"SIGTERM"` + `shutdown signal received — draining`, no `running` `job_run` rows left, exit 0 within seconds, the sibling keeps scheduling (rows 32 → 40) and is the only `himma_worker` connected afterwards; SIGTERM to an API container → handler logs the signal, drains, pool closed (one `himma_api` backend set remains), exit 0, sibling stays ready. PID 1 is `node --import …/loader.mjs scripts/start-*.ts` (uid 10001, ppid 0) — nothing between the runtime and the application to swallow a signal.
- **Exposure:** only `api` publishes ports (host loopback 8080/8081 for the harness checks); `postgres` and `worker` unpublished; `migrate`/`verify`/`provision-roles`/`maintenance` are exited one-shots, never resident.
- **CI parity:** build command/context/Dockerfile identical to `ci.yml`; the CI invariant steps were executed locally under bash (non-root user; no `.env`/dev scripts/tests; CA present; all six invocations load from production dependencies and refuse without `DATABASE_URL` under `--read-only`); Trivy CRITICAL/HIGH `--ignore-unfixed` → 0 findings on the final image; `npm audit --omit=dev --audit-level=high` → clean.

**Files changed (and why):** `backend/Dockerfile` (CA directory modes; in-process entrypoint + `TSX_DISABLE_CACHE`; drop npm/yarn/corepack; digest pin) · `backend/scripts/cli-env.ts` (lazy dev-only `dotenv`) · `backend/src/config/runtime.ts` (production refuses non-TEST-mode Stripe keys at startup) · `backend/package-lock.json` (`fast-uri` 3.1.7/4.1.4 only) · `backend/test/runtime-config.test.ts` (+1 test) · `backend/test/infra-locks.test.ts` (entrypoint/cache/chmod/compose read-only assertions) · `infra/local/docker-compose.yml` (`read_only: true` on every backend container; `postgres-unlisted` alias for the hostname-mismatch proof) · `infra/local/harness.sh` (full certification transcript: 34 proofs) · `.github/workflows/ci.yml` (invariant step covers all six invocations under `--read-only`; shell-agnostic) · `infra/README.md` §11 (execution record, digests, runtime) · docs/36 (note only — no row changes) · HANDOFF.

**Launch-gate impact:** **none.** The distinction now recorded: IaC/code complete → **container artifact CERTIFIED (locally, arm64; CI repeats on amd64)** → staging NOT deployed → staging smoke NOT performed → production NOT deployed. docs/36 counts unchanged (79 open · 59 P0 · 20 P1). `productionChargingPossible` = literal `false`. Migration head `0023_job_run_and_maintenance_authority`. Zero AWS resources applied.

**Verification (this commit):** backend Jest **1413/1413 across 123 suites** (serial `--runInBand` run, 0 race errors; parallel runs hit the pre-existing XX000 provisioning race described above — 1404/1413 and 1403/1413 with that single error class) · backend tsc/ESLint clean · `db:verify` 23/0 (native dev DB) and 23/0 from the image · focused: `infra-locks` 12/12, `runtime-config` 12/12, `db-tls` 8/8, `production-api-process`, `worker-process` 6/6, `scheduler-process` 7/7 · `terraform fmt -check` + `validate` GREEN (stack + bootstrap) · harness HARNESS PASSED (34 proofs) · Trivy 0 CRITICAL/HIGH · `npm audit` gate clean. Frontend suites NOT rerun: no frontend-facing source changed (backend runtime/config, Dockerfile, harness, CI, docs only). **Pre-existing test-farm race surfaced during this correction (NOT introduced by it; recorded as follow-up RI-7):** nine suites call the cluster-wide `provisionRuntimeRoles` (`ALTER ROLE himma_api/himma_worker/himma_maintenance_runner`) from parallel Jest workers; when two overlap PostgreSQL raises `tuple concurrently updated` (SQLSTATE XX000) and the affected suite fails. Reproduced at the untouched baseline `6f53800` with the working tree stashed (full run 1406/1412, `worker-process` failing on exactly this error) and in full runs of this tree (the same single error class); every affected suite passes alone and the same runs are otherwise green. The provisioning boundary itself is certified W6-1 code and is NOT changed by this correction (scope lock); the follow-up is a test-side serialization of cross-worker role provisioning (or a bounded retry on XX000 in the boundary) — owner's call. The process-spawning suites are also CPU-timing-sensitive: never run them while the container harness is running on the same machine.

## W6-4A · RUNTIME-ROLE PROVISIONING CONCURRENCY — FINAL OWNER-CORRECTION RECORD (2026-09-08 at `cba2bd8`; on top of `06e8c6e`; OWNER-APPROVED / CLOSED at `cba2bd8` (2026-09-08))

**Blocker:** the container artifact at `06e8c6e` is accepted; the cluster-global runtime-role provisioning raced under concurrent invocation (`tuple concurrently updated`, reproduced at the untouched baseline), and W6-4A owns the CI foundation. Also: the tree carried an untracked `AGENTS.md`.

**Root cause (from source and PostgreSQL behaviour):** `provisionRuntimeRoles` took `pg_advisory_lock(hashtextextended('himma:provision-roles', 42))` on its ADMIN connection — the connection to the caller's own database. PostgreSQL advisory locks are DATABASE-SCOPED: `pg_locks` records the acquiring session's database as the lock target, so the same key held in `himma_test_a` does not block a session in `himma_test_b` (proven empirically: a second session in `postgres` acquired the key while a session in `himma_backend_dev` held it; `pg_locks` showed `locktype=advisory database=himma_backend_dev`). The roles it protects (`himma_api`, `himma_worker`, `himma_maintenance_runner`, and the migration-created `himma_app`/`himma_maintenance`) are CLUSTER-GLOBAL catalog objects (`pg_authid`, `pg_auth_members`). Under the repository's real topology — every suite provisions its own `himma_test_*` database and nine suites call the boundary — parallel Jest workers (bare `npx jest`; the repository's `npm test` is `jest --runInBand` by the docs/25 §8 shared-server policy and never raced) issued `ALTER ROLE`/`GRANT` on the same catalog tuples simultaneously; PostgreSQL's catalog update path does not row-lock those tuples and raises `tuple concurrently updated` (XX000). The same race exists for any two real provisioners on one cluster (two migration run-tasks, an operator plus a pipeline). Reproduction against `06e8c6e` (standalone script, three runs each): 12 databases × 16 simultaneous provisioners → 12–13 of 16 failed; 10 × 10 → 8–9 of 10; 8 × 8 → 6–7 of 8; 6 × 6 → 5 of 6 — every run.

**Fix (`src/db/provision-runtime-roles.ts`; provider-neutral, idempotent, secret-safe, fail-closed):** the lock is taken on a SEPARATE session to ONE canonical coordination database of the cluster — the maintenance database `postgres` that every PostgreSQL cluster, the CI service container, the harness, and every RDS instance have — using the same host/port/admin credential; it is held for the whole run and released with the session. Every provisioner therefore shares one lock space regardless of which database its admin connection targets. `HIMMA_PROVISION_LOCK_DATABASE` (script env → `coordinationDatabase`) overrides the database name; if the coordination session cannot be opened the run fails closed with a message naming the database and the override — never a silent database-local lock. Work under the lock is unchanged (no privilege broadened, no password in any migration, no AWS-specific mechanism, no external coordinator). Same reproductions after the fix: 16/16 and 10/10 succeed on every run.

**Proof (`test/provision-runtime-roles-concurrency.test.ts`, real PostgreSQL, 5 tests):** 6 independent migrated databases, 6 simultaneous provisioners (12 peak sessions, two per provisioner: coordination + admin) → all fulfilled, zero XX000, every invocation touched all three logins; exact membership matrix `himma_api→himma_app`, `himma_worker→himma_app`, `himma_maintenance_runner→himma_maintenance` and nothing else; attributes exact (three LOGIN runtime roles, two NOLOGIN privilege roles, none SUPERUSER/CREATEDB/CREATEROLE); a second simultaneous wave is all-update/no-create with the topology unchanged; holding the key in the coordination database blocks a provisioner until released (the serialization demonstrably lives there); a non-existent coordination database fails closed; after concurrent provisioning `himma_api`/`himma_worker` are denied DDL, `SET ROLE himma_maintenance`, `DELETE` and the SECURITY DEFINER retention function, `himma_maintenance_runner` is denied DDL, `SET ROLE himma_app`, `DELETE` and `pgmigrations` writes, and no login owns schema objects (migration authority stays separate).

**Container regression (the provisioning module ships in the image):** image rebuilt; `infra/local/harness.sh` gained a step that launches 8 simultaneous `db-provision-runtime-roles` one-shots from the image (the deployment command) against the harness PostgreSQL and asserts all exit 0, the exact membership matrix, and no cluster-privilege broadening; the full harness was re-executed — HARNESS PASSED, 35 proofs (image `sha256:548384d1…`; the 8 simultaneous image provisioners all exited 0; memberships exactly `himma_api→himma_app`, `himma_maintenance_runner→himma_maintenance`, `himma_worker→himma_app`; no runtime role gained SUPERUSER/CREATEDB/CREATEROLE; api ×2 / worker ×2 / maintenance identities unchanged); Trivy CRITICAL/HIGH 0.

**Backend test topology, stated plainly:** `npm test` (the CI command) is `jest --runInBand` — serial by the pre-existing docs/25 §8 policy for one shared local server, NOT introduced to hide this race; it passed before and after. The acceptance proof requested by the owner is the NORMAL PARALLEL execution (`npx jest`): with the boundary fixed, every parallel run since (15 runs) shows **zero** `tuple concurrently updated` (before: 12–20 per run). A second, distinct property was then measured and bounded: at the default one-worker-per-core topology (10 workers here) the suite peaks at **91–100 client backends** against a default `max_connections=100` server (sampled every 200 ms during the runs) — every suite may burst to a 10-connection pool and the process-spawning suites add their children's pools — so unrelated suites intermittently failed with `too many clients` (1 of 3 runs even without the new proof present was at 93–100; with it, 2 of 3). Shortening the test pools' idle timeout did not move the peak (91–98; reverted). The bounded, justified fix is `maxWorkers: 4` in `jest.config.cjs` (a genuinely concurrent topology, not serialization): three consecutive parallel runs **1418/1418 across 124 suites** at peaks of 68/76/69 backends, zero race errors, zero capacity errors, ~38 s each. `npm test` (serial, the CI command) 1418/1418 on every run. No test was weakened, skipped, retried, or marked allowed-to-fail; `ci.yml` is unchanged.

**`AGENTS.md`:** provenance established — byte-identical to the tracked `CLAUDE.md` apart from the substitution "Claude Code" → "Codex", created 2026-09-08 09:33 outside this session (an owner-side Codex instruction mirror). Not committed (the owner did not ask for a tracked Codex mirror and it duplicates `CLAUDE.md`); preserved verbatim outside the repository at `/Users/abdu/Himma-untracked/AGENTS.md`. If the owner wants Codex instructions tracked, `git add` of that file is a deliberate decision, not repository hygiene. `git status --short` is empty.

**Files changed:** `backend/src/db/provision-runtime-roles.ts` (coordination session + fail-closed), `backend/scripts/db-provision-runtime-roles.ts` (`HIMMA_PROVISION_LOCK_DATABASE`), `backend/.env.example` (documented optional variable), `backend/test/provision-runtime-roles-concurrency.test.ts` (new, 5 proofs), `backend/jest.config.cjs` (`maxWorkers: 4`, measured capacity rationale), `infra/local/harness.sh` (8-way concurrent provisioning proof from the image), `infra/README.md` §7 (coordination database note), docs/38 (this record), docs/36 (note only), HANDOFF.

**Launch-gate impact:** none — no new gate for a defect fixed inside the owning slice; no deployment/staging gate closed. docs/36 counts unchanged (79 open · 59 P0 · 20 P1). `productionChargingPossible` = literal `false`. Migration head `0023_job_run_and_maintenance_authority`. Zero AWS resources.

**Verification (this commit):** focused `provision-runtime-roles-concurrency` 5/5 · `infra-locks` 12/12 · normal parallel backend Jest **1418/1418 across 124 suites ×3 consecutive runs** (zero race, zero capacity errors; peaks 68/76/69 backends) · `npm test` (serial, CI command) **1418/1418** · backend tsc/ESLint clean · `db:verify` 23/0 · image rebuilt + harness HARNESS PASSED (35 proofs) · Trivy 0 CRITICAL/HIGH · Terraform unchanged (fmt/validate GREEN at `06e8c6e`). Frontend suites not rerun: no frontend-facing source changed.

## W6-4B · Deploy and Certify Himma STAGING — RECORD (2026-09-08, this commit): BLOCKED AT THE EXTERNAL OWNER PREREQUISITE; repository-side preparation COMPLETE

**Authority:** owner authorization W6-4B (2026-09-08) on the closed W6-4A chain (`0393601` → `6f53800` → `06e8c6e` → `cba2bd8`; closure bookkeeping `31a9998`).

**Reconciliation (before any action):** HANDOFF, docs/36, docs/37, docs/38, `infra/README.md`, the Terraform stack + bootstrap, the Dockerfile, `ci.yml`/`deploy.yml`, the runtime configuration contract (`DATABASE_*`, `COGNITO_*`, `EVIDENCE_S3_*`, `PAYMENTS_MODE`/`STRIPE_*`, peppers), the Cognito contract (public clients, `USER_PASSWORD_AUTH`/`REFRESH_TOKEN_AUTH`), the evidence-storage contract (SigV4 driver, VE-02 content safety absent → fail-closed), and git through `cba2bd8` were inspected. Nothing in the closed architecture was redesigned.

**Real AWS access — discovered, not assumed (non-secret facts):** no `aws` CLI was installed (installed `aws-cli/2.36.40` as ordinary tooling so the preflight and the owner runbook are executable); `~/.aws` does not exist; no `AWS_*` environment variable; no SSO cache; no Terraform var file or backend config beyond the `.example` templates; no Terraform state anywhere. **This repository has no git remote** (GitHub CLI signed in as `abd724`; nothing pushed) — so no CI has ever run and no GitHub environment/OIDC trust can exist yet. **Consequence:** there is no authenticated principal, no account id, no region session, no state bucket, no hosted zone, no GitHub repository — the hard preflight cannot even reach the identity check (`tf-preflight.sh staging` → exit 4 "no authenticated AWS session — nothing will be applied"). Per §4 of the directive: nothing fabricated, no personal/unrelated account used, no `terraform plan/apply`, no AWS resource of any kind created, production untouched.

**Repository-side preparation completed in this commit:**
- **Preflight proven** (`backend/test/infra-preflight.test.ts`, 7 tests): the REAL `tf-preflight.sh` runs against a temporary copy of the infra tree with a stub `aws` on PATH (no credential, no network; the operator's gitignored var file is never touched) — missing var file → exit 2; wrong region in the var file → exit 3 before AWS is consulted; no CLI / no session → exit 4; account mismatch → exit 5 with the exact "REFUSED: authenticated account X != expected Y" line and never a re-target path; right account in the wrong session region → exit 5; the single OK path prints only identity facts; production → exit 6 without `HIMMA_ALLOW_PRODUCTION_APPLY=yes`, and the staging account can never pass a production preflight even with it.
- **`infra/scripts/ecs-deploy.sh provision`** — the runtime-role provisioning as an ECS one-shot on the `migrate` task definition (schema authority + the three runtime passwords the task already reads from Secrets Manager; the cluster-wide provisioning lock of `cba2bd8` applies), completing the migrate → verify → provision sequence of infra/README §4 step 9 as executable commands.
- **`infra/scripts/static-deploy.sh <env> <portal|admin|web>`** — builds the portal/admin with the PUBLIC `frontend_build_env` outputs, refuses if the built output carries a secret-shaped string (before any upload), syncs to the private origin with immutable asset caching + revalidated `index.html`, invalidates CloudFront; production refused without the explicit flag.
- **`infra/scripts/staging-certify.sh`** — the W6-4B certification transcript against the REAL staging account: HTTPS/redirect/certificate chain, live/ready (no internals in the body), dev identity 404, forged bearer 401, independent request ids across 6 requests; ECS topology (api + worker services only, worker without load balancer, ≥2 healthy ALB targets, api ×2 / worker ×2); DB sessions from INSIDE the deployed image as one-shot tasks (identity `himma_api`/`himma_worker`/`himma_maintenance_runner`, `sslMode=verify-full`, `ssl=true`, statement_timeout 15s/1min/5min, UTC, migration head `0023…`); privilege boundary from inside the image (no DDL, no `pgmigrations` writes, no `DELETE`, api/worker cannot assume maintenance, runner cannot assume `himma_app`); migration task at head = no-op + `db:verify`; maintenance `retention.all` + unknown-job exit 2 + the EventBridge schedule targets only `retention.all`; CloudWatch ingestion (worker running line with both loops, scheduled job runs, no secret-shaped strings); RDS actual state (private, encrypted, `rds.force_ssl=1`, version/backups/storage/monitoring printed); then the security review.
- **`infra/scripts/aws-security-review.sh <env>`** — actual-state review by AWS API: zero IAM users, GitHub OIDC provider present, every `himma-<env>*` bucket blocks all public access + TLS-only policy + encryption, RDS not public + encrypted, only the ALB security group world-open and only on 80/443, the RDS security group admits no CIDR, api/worker the only services (worker no load balancer), every task definition read-only root + uid 10001, **simulated IAM evaluation**: each execution role reads only its own DB secret and never the RDS master secret, only the API task role can write evidence, and no task definition carries a live payment mode.
- **Staging public configuration examples** (`.env.staging.example`, `portal/.env.staging.example`, `admin/.env.staging.example`): the exact PUBLIC names the three frontends consume, filled from `terraform output frontend_build_env`; Apple/Google sign-in flags stay `false` (ID-05…07 external).
- **Owner runbook `docs/39_STAGING_BOOTSTRAP_OWNER_RUNBOOK.md`**: the prerequisites (accounts, non-root engineering identity, domain/zone, GitHub repository + environments), the exact non-secret values and where each goes, the owner console steps, and the ordered engineering commands (SSO profile → push to GitHub → var file → state bootstrap → preflight → plan/review/apply → immutable image → secrets → migrate/provision → rollout → scale to 2 → certification → static deploy → GitHub variables → CI/CD), plus the failure drills and what stays open afterwards.

**External blockers (owner-controlled; exact minimal steps in docs/39 §3–§4):** (1) the `himma-staging` workload account under the Organization; (2) a non-root engineering identity in it (Identity Center permission set or assumable role) and its SSO start URL/role ARN; (3) the company domain and its hosted zone id (or permission to create/delegate); (4) the GitHub repository with `staging`/`production` environments and the first push; (5) later, per gate: Stripe TEST credentials, an alert destination, Apple/Google credentials.

**Launch-gate impact:** none. No gate closes or advances on repository preparation alone; docs/36 counts unchanged (79 open · 59 P0 · 20 P1). Zero AWS resources. Production untouched. `productionChargingPossible` = literal `false`. Migration head `0023_job_run_and_maintenance_authority`.

**Verification (this commit):** `infra-preflight` 7/7 (new) · `infra-locks` 12/12 · backend `npm test` **1425/1425 across 125 suites** (the 1418 baseline + 7 preflight proofs) · normal parallel run 1425/1425 · backend tsc/ESLint clean · `db:verify` 23/0 · new scripts `bash -n` clean · Terraform unchanged (fmt/validate GREEN at `cba2bd8`). Frontend suites not rerun: no frontend source changed (public `.env.staging.example` templates only).

**Owner state (obvious):** DEPLOYED — nothing in AWS. NOT DEPLOYED — everything (no account exists). BLOCKED — on the five owner prerequisites above. NEXT ACTION — owner: docs/39 §3 (accounts, identity, domain, GitHub); then engineering resumes at docs/39 §4 step 1.
