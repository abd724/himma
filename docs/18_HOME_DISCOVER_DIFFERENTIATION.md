...
Status: product-owner approved, 2026-08-03 (including the §6 child-dependent collection visibility rule, approved the same day). Approved decisions in this document supersede the conflicting parts of docs/04 HMA-004, docs/09 §17.4, and docs/11 listed in §13. Nothing here changes the approved visual system: Orbit Indigo tokens, typography, floating dock, card language, imagery, spacing, and native navigation are untouched (docs/07, docs/11 §7, docs/12).
...
|---|---|---|
| Age-appropriate children's content | `For {participantName}` — generated per real child profile, eligibility- and interest-ranked | `After school`, `Kids & Teens` — broad collections over the whole catalogue (visibility per §6) |
...
- Sarah, Adam, and Lina are deterministic mock profiles of the default demo account only. Components accept dynamic participant arrays; tests must cover zero-children, one-child, and multiple-children accounts (§15).
...
### Removed from Home (see §13 for spec supersessions)

| Section (docs/11 §4) | Fate |
|---|---|
| Participant context chips (`Everyone · Me · Adam · Lina`) | **Removed from Home.** Home shows every participant's content simultaneously via labelled sections; no profile switching needed (§8). |
...
| Seasonal hero (`Indoor this August`) | **Removed from signed-in Home.** Upcoming activity leads instead. Seasonal editorial lives in Discover collections. A seasonal welcome card may appear only on guest/no-history Home (§9), where no upcoming activity exists. |
...
7. Child-focused content is additionally gated by account composition per §6.

## 6. Child-dependent collection visibility (approved owner rule, 2026-08-03)

An account containing only the primary participant (`Me`) must **not** automatically see child-focused or student-focused collections. This is a **visibility rule, not a ranking preference**.

Hidden by default when the account has no child profiles:

- Summer camps, winter camps, holiday camps
- After-school activities
- Kids & Teens collections
- Child development rails
- Any other collection whose intended participant is a child or school-age student

Child-focused collections may appear only when **both** conditions hold:

1. the account contains at least one child profile; **and**
2. the collection has meaningful provider-defined age-eligible supply for at least one of those children.

An empty or irrelevant child-focused rail must never render merely because a child profile exists.

### Home behavior

For a `Me`-only account:
...
Discover remains the broad marketplace, but child-focused editorial content is not promoted by default to a `Me`-only account:

- **`Me`-only account:** child-focused collections are hidden from the default Discover feed; adult-suitable categories and collections stay prominent. Child-focused content remains reachable through deliberate search and explicit catalogue browsing (All Categories and category pages stay complete — hiding applies to promoted feed placement, not the catalogue).
- **Child participant selected:** child-focused collections may appear; the selected child's age eligibility applies; adult-only programs are hidden (existing docs/16 §4 rules).
- **Everyone selected and child profiles exist:** child-focused and adult-focused content may both appear, in clearly labelled sections.

Collection data gains an intended-participant classification so this rule is data-driven (the current `Camps & holidays` and `After school` collections are child-intended; `Kids & Teens` is child-intended by definition). No logic may branch on collection titles.

## 7. Discover: exclusive ownership
...
- The Home header may later gain a compact household/participant action (e.g., avatar → Participants); this is optional and deferred (§18). Home must never require profile switching to see each person's content.
- Checkout-time participant selection remains a separate concern (docs/02 §8); nothing on Home books on anyone's behalf.

## 9. Account scenarios and dynamic behavior

Home renders correctly for any participant list and booking state. Four normative scenarios (each must be demonstrable deterministically, §14):
...
## 10. Upcoming bookings and weekly schedule (deterministic mock)
...
## 12. What Discover loses / gains

Discover's screen hierarchy, navigation, and visuals stay exactly as approved and implemented (docs/14–16). Two behavioral additions from this document:

1. The default Discover feed becomes **account-composition-aware** per §6: for a `Me`-only account, child-intended collections are excluded from the promoted collections rail (search and catalogue browsing remain complete).
2. Discover gains exclusive ownership of the marketplace surfaces listed in §7.

The shared area context (location chip) continues to apply to both screens.

## 13. Contradiction review and supersessions
...
| docs/09 §17.4 (Home quick filters re-filter in place) | Home no longer has a quick filter row | **Superseded for Home** (owner decision, 2026-08-03, recorded here and in docs/09 §18). Discover quick-chip semantics are unaffected. |
...
| docs/14 §2.6 (Collections rail, conditional) | Conditionality was supply-based only | **Extended by §6 here:** collection visibility now also depends on account composition (child-intended collections require a child profile with age-eligible supply). |
...
| docs/15 §3.12 (Kids & Teens / Camps as prominent browse entries) | Prominent by default for every account | **Qualified by §6:** promoted-feed prominence requires child profiles with eligible supply; All Categories and deliberate browsing remain complete for every account. |
| docs/16 §2, §4 ("Discover feed follows Home §5"; participant context shared by Home and Discover) | Home no longer consumes the browsing context | Context tables in docs/16 §4 now govern **Discover/Search/Results only**; the app-level context remains shared across those surfaces. Home is context-complete per §8 here. §6's account-composition gate composes with (does not replace) those context rules. |
...
Existing Home state rules (docs/11 §9) apply unchanged: loading skeletons (header/dock immediate), image fallbacks, reduced motion, non-color selection indicators. Additional required states: each §9 scenario (A–D) demonstrable deterministically; week strip with 0 sessions on some days; routine card at different progress values; collapse of any participant rail with thin supply; the `Me`-only Discover feed with child-intended collections hidden (§6). No empty-state fabrication: a scenario with no upcoming bookings simply has no upcoming/week sections rather than an empty-schedule placeholder.
...
- Child-dependent visibility (§6), required cases:
  ...
  - no logic branches on fixed child names or collection titles.
...
- Existing Discover/Search/Results/catalogue/map tests and QA scripts must continue to pass unchanged except where §6 alters the default Discover collections rail; Home QA script and screenshot baselines are updated to the new hierarchy.

## 16. Accessibility and native compatibility
...
## 17. Acceptance criteria (for the Home rework commit)

1. Home renders the §4 hierarchy; every removed section from §4 is absent; no participant chips or quick filter row on Home.
2. All four §9 scenarios demonstrable deterministically; no fabricated data in A/B.
...
6. Child-dependent visibility holds on Home **and** the default Discover feed per §6, with all §15 visibility cases passing.
7. Visual system unchanged: tokens, typography, dock, card anatomies, spacing; no new visual language beyond §11's compositions.
8. §15 tests pass; tsc, lint, jest, expo-doctor, QA scripts green; Discover surfaces unaffected except the §6 collections-rail behavior (their other screenshots byte-identical).

## 18. Open decisions

- Guest promotion of child-focused collections: a guest has no account signal at all. Temporary assumption until decided: guests see the broad marketplace default on Discover (collections unfiltered), while guest **Home** stays neutral (no child-focused personalized rails exist there anyway, since there are no profiles). Recorded as an assumption, not a rule.
