...
**Status: owner-review specification (2026-08-07). Documentation and technical design only — no migration, service, route, portal, or frontend code accompanies this document, and its approval starts nothing by itself.**
...
**Storefront ownership chain (binding).** `Organization → OrganizationPublicProfile (storefront) → Branch → future Listing (Program, Slice 4)`. Search later operates at the listing level across providers; every listing must resolve its owning organization and public storefront in one hop (`Program.organization_id` already binds this in docs/24 §1.4), and a storefront must enumerate its organization's published listings by one indexed query. Slice 3 builds the ownership spine; it deliberately builds NO listing schema — §12.8 proves the spine admits the docs/24 Program shape without redesign.
...
8. Verification-status foundation: the §5.1 state machine plus the minimal admin transition API. (The `VerificationCase` checklist/notes/document entity and the full W3 verification workflow are deferred — owner decision D-S3-3.)
...
| `org_kind` text CHECK (`provider`) | Classification seam. Only `provider` exists — `partner` is D-S3-4, not silently invented. |
...
**Visibility rule:** a storefront is publicly readable IFF `organization.verification_state = 'live'`. Existence of an organization row never makes it discoverable (§10). Verified indicator: derived in the read model from state (`live` ⇒ verified badge); never a settable boolean. Ratings/reviews: later slice; the projection reserves the field names as absent-until-built (mock mapping §15).
...
- **Accept** (authenticated route): the caller is an authenticated Himma user (existing user signs in; a not-yet-existing user first completes normal Cognito first-login, which creates the canonical `User` — invitation acceptance never creates a parallel identity). Transaction: validate digest + expiry + state → verified-email match per D-S3-1 → CAS `sent → accepted` (single-use; the CAS admits exactly one winner under concurrent acceptance) → insert `staff_membership` active (+ scope rows) → audit + outbox `staff.joined` — one transaction (docs/24 §7.10). Wrong-user acceptance (verified-email mismatch, D-S3-1 recommendation) → typed `invitationInvalid` without revealing the intended recipient; the invitation stays `sent`.
...
2. **Organization creation** (Slice 3): a Himma admin (`operations` role, existing `admin` policy surface) creates the organization in `draft` with its public-profile shell and issues the FOUNDING Owner invitation in the same transaction (the only membership whose `invitation_id` provenance is admin-recorded). Provider self-service organization creation is deliberately NOT built (D-S3-2 records the recommendation; revisit with the W2 portal spec).
3. **Verification** (Slice 3 = state machine + admin transition API only): provider owner submits (`draft → submitted`, completeness-validated: profile essentials + ≥1 branch); admin operations moves `submitted → in_review → verified | rejected`; `rejected → submitted` on resubmission. The checklist/document/notes tooling (`VerificationCase`) is deferred (D-S3-3).
4. **Go-live** (Slice 3): `verified → live` is an EXPLICIT admin action — never automatic. Only `live` makes the storefront publicly readable; no organization becomes discoverable by existing (§3 visibility rule). `live → suspended → live` and `→ offboarded` per §5.1.

## 11. Provider vs partner

**One focused owner decision (D-S3-4), with recommendation:** do NOT create a separate canonical `partner` organization type in Slice 3. Basis: docs/24's canonical entity is `Organization` with `commercial_terms_ref` (§14.B4, owner-open) — "partner" as encountered so far is a commercial/relationship classification, not a differently-shaped entity or a different permission model. The schema seam is `org_kind` (CHECK `'provider'` only today): if a future ruling defines partners as materially different, a value + targeted columns arrive additively. Inventing a partner permission model without owner approval is explicitly out.
...
4. Uniques/indexes: partial unique active membership `(user_id, organization_id) WHERE state='active'` · unique `token_digest` · partial index live orgs (public read) · `branch (organization_id, active)` · `staff_membership (organization_id, state)` · `staff_invitation (organization_id, state, expires_at)`.
...
**13.1 Customer-public storefront read foundation (policy `public`):**
- `GET /providers/:organizationId` → the public projection ONLY: display name, description, media refs, verified indicator, public contact fields, active branches (label, area label, address line, geo, hours, facilities). Served exclusively from `organization_public_profile` + public `branch` columns; non-`live` orgs are not-found-shaped. Listings composition, ratings, and search integration arrive with Slice 4 — the projection is forward-shaped for them (absent fields, not fake ones).

**13.2 Provider-private management (policy `provider` + capability; org-addressed; suspended-org mutation refusal):**
...
- `GET /provider/organizations/:orgId/staff` (owner) · `POST .../staff/invitations` (owner, `stepUpRequired`) · `POST .../staff/invitations/:id/revoke` (owner, `stepUpRequired`) · `POST .../staff/memberships/:id/revoke` (owner, `stepUpRequired`).
...
- `POST /admin/organizations/:orgId/verification/(start-review|verify|reject)` · `POST /admin/organizations/:orgId/(go-live|suspend|reinstate|offboard)` — §5.1 admin edges, CAS + typed outcomes, each audit/outbox-evented. Checklist/document tooling deferred (D-S3-3).
...
6. Invitation races: concurrent acceptance of one token → exactly one winner; revoked/expired/accepted/mismatched-email all → byte-identical `invitationInvalid`; expiry sweep idempotent + evented (B2-5 expiry pattern).
...
8. Authorization matrix: every role × every Slice-3 route (capability grid); customer (no membership) → provider routes not-found-shaped; provider staff → every `/admin/*` route `forbidden` (no admin rows); admin principal → provider-private routes refused without membership; Cognito claims grant nothing (existing guard extended).
9. **Public/private separation:** structural test locks the §13.1 projection to `organization_public_profile` + public branch columns; response-shape test proves no legal_name, no private field, no staff data in any public payload; non-live orgs not-found-shaped.
...
|---|---|---|---|
| S3-1 | `feat(backend): provider organization schema` | Migration 0005: organization + public profile + branch, §5.1 machine, triggers, grants, codegen, SCHEMA_CHECKS | Machine edge tests, isolation FKs, public/private structural split, §12.8 forward-compat test; backend tsc/lint/focused jest + migrate-from-zero + `db:verify` + rollback → **stop** |
| S3-2 | `feat(backend): staff memberships and invitations` | staff_membership(+branch scope) + staff_invitation tables (same migration file if S3-1 unapplied, else 0006), §5.2 machine, last-owner guard, invitation services (issue/accept/revoke/expire sweep) with token-digest boundary + MailSender capture, audit/outbox | §14.5–6 races + hygiene; focused suites green → **stop** |
| S3-3 | `feat(backend): provider principal and management routes` | `provider` policy category, orgScope resolution, capability registry, §13.2 routes, typed outcomes, rate limits | §14.3–4, §14.8 matrix, policy snapshot; full backend suite → **stop** |
| S3-4 | `feat(backend): admin org lifecycle and public storefront read` | §13.3 admin routes (behind the existing admin policy + production gate), §13.1 public projection route | §5.1 admin-edge route tests, §14.7 suspension matrix, §14.9 projection lock; full backend suite → **stop** |
...
**Block the Slice-3 schema (needed before S3-1/S3-2):**
- **D-S3-1 · Invitation acceptance binding.** Recommendation: acceptance requires the accepting user to hold a VERIFIED identity email matching the invitation email (mismatch → `invitationInvalid`); prevents forwarded-token takeover. Alternative (any authenticated holder of the token) is weaker and not recommended.
- **D-S3-2 · Organization creation path.** Recommendation: admin-initiated only in Slice 3 (draft org + founding Owner invitation, §10); provider self-service signup is specified with the W2 portal after design-partner discovery (docs/23 §8.6).
- **D-S3-3 · Verification tooling split.** Recommendation: Slice 3 = §5.1 state machine + minimal admin transition API; `VerificationCase` (checklists, notes, documents) and workflow tooling move to the admin workstream. This narrows docs/24 §13 row 3's letter ("VerificationCase, admin verification workflow") and therefore needs explicit approval; the state machine and typed admin API keep row 3's acceptance drivable end-to-end.
- **D-S3-4 · Partner classification (§11).** Recommendation: no separate canonical type; `org_kind` seam + later commercial classification via §14.B4.
...
**Can wait for the provider portal UI (W2):** provider MFA/step-up policy beyond the §8 baseline (D-S3-5: whether provider logins require MFA like admins — baseline: step-up on staff management/offboarding only) · expression-of-interest page + self-signup · org switcher UX · bulk tooling · any read-only role addition · design-partner-driven contract adjustments (docs/23 §8.6 gates provider-API freeze).
