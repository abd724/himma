...
| **W2-3** Onboarding & verification status | **Implemented 2026-08-07** — invitation acceptance at `/invitation/:token` (canonical single `invitationInvalid` collapse; auth composed via returnTo; token hygiene proven); onboarding hub at `/o/:organizationId/onboarding` with the §9 checklist over canonical `verification_state`, capability-derived actionability, submit/resubmit (`organizationIncomplete`/`staleVersion`/CAS mirrored), honest "Himma will contact you" documents messaging (no upload UI); shell setup bar for non-live orgs; invitation/onboarding ports for W2-12; new fixture orgs across draft/submitted/in_review/rejected/verified. Recorded contract gaps: no invitation preview read (B), founding-owner distinction is composition-only (A), no provider-facing rejection reason (B). Verification/go-live stays fail-closed and admin-controlled; W3-minimum admin surfaces + evidence backend remain outreach dependencies. Jest 191/191 incl. jest-axe; Playwright 48-pass at 1440/1024/390; evidence in `artifacts/portal-w2-3/`. Awaiting owner review. |
| W2-4 … W2-13 | Not started — sequential, each gated on owner review of its predecessor. |
