...
**W6-3 status note (this commit):** the inventory below is the LR-0 snapshot. As of W6-3 every non-destructive runner in it is scheduled by the worker-hosted advisory-lock scheduler (`payment.checkout-sweep` 60 s · `payment.reconciliation` daily · `payment.stuck-state` 5 min · `booking.hold-sweep` 5 min · `identity.role-expiry` hourly · `provider.invitation-expiry` hourly; the W6-2 outbox/search/gateway-event/trusted-result loops continue), and the engineering-scoped retention sweeps run only in the short-lived `maintenance` mode under `himma_maintenance_runner`. Policy-gated retention (audit, identity records, evidence) stays absent pending VE-05/SE-05. Deployment mechanism for all rows remains the open IN-01/IN-11/IN-12 track.
