...
|---|---|---|---|---|---|---|---|
| ID-01 | Production Cognito User Pool provisioned (region per IN-01) | EXTERNAL-DEPENDENCY (ADVANCED by W6-4A: the per-environment Cognito user pool + three public app clients matching the certified contract are IaC-complete (`infra/terraform/modules/cognito`); the pool exists only after the staging/production applies) | P0 | IN-01 | M2 | Owner/company (AWS) | docs/26 §1 (no pool exists, recorded not guessed). Proof: pool + issuer recorded in ops config inventory |
...
| IN-04 | Secret management: managed store delivering env vars (backend is env-only by design; `loadConfig` fail-closed — production refuses missing `DATABASE_URL`) | CONFIGURATION-REQUIRED (ADVANCED by W6-4A: the managed-store boundary is IaC-complete — Secrets Manager containers per consumer, per-task execution roles scoped to their own secret ARNs, `DATABASE_PASSWORD` companion for managed-secret injection, no value in Terraform state/source; values and the deployed proof arrive in W6-4B) | P0 | IN-01 | M1 | Ops | `config/env.ts:123-132`. Proof: secrets sourced from the managed store, none in code/images |
| IN-05 | Production PostgreSQL at the financial service class (docs/23 §10.11: RPO 0 committed transactions, quorum/PITR) | EXTERNAL-DEPENDENCY (ADVANCED by W6-4A: RDS PostgreSQL 16 Multi-AZ/PITR/encrypted/TLS-forced/deletion-protected is IaC-complete (`infra/terraform/modules/database`); provisioning + the recorded posture arrive with the staging (W6-4B) and production applies) | P0 | IN-01 | M1 | Ops | Proof: provisioned instance + replication posture recorded |
| IN-06 | Production object storage bucket + IAM (the S3 driver is CODE-COMPLETE and never composed anywhere — verified: only caller is its test) | CONFIGURATION-REQUIRED (ADVANCED by W6-4A: private evidence bucket (block-public, versioned, SSE, TLS-only, prefix-scoped IAM, no lifecycle expiry) is IaC-complete; the real-bucket smoke (VE-01) needs the staging apply; content safety (VE-02) untouched) | P0 | IN-01 | M1 | Ops | `s3-evidence-store.ts:63` callers. Proof: composed in IN-02; VE-01 smoke |
| IN-07 | **Distributed rate-limit store.** **CLOSED by W6-1** (OWNER-APPROVED at `e01af85`): `PgRateLimiterStore` behind the existing `consume` port over migration `0021_rate_limit_window` — atomic DB-clock windows, multi-connection/multi-pool/multi-PROCESS sharing proven, restart-durable, digest-bounded keys, in-memory store still refuses production | CODE-COMPLETE | P0 | IN-02 | M1 | Engineering (W6) | docs/37 W6-1 record; `test/pg-rate-limiter.test.ts` + the spawned two-process 429 proof |
...
| IN-09 | Structured logging + request-id propagation. **CLOSED by W6-1** (OWNER-APPROVED at `e01af85`): server-generated canonical ids (client hints bounded, never adopted), AsyncLocalStorage context, the `appendAuditEvent` ambient seam populating `audit_event.request_id` end-to-end (customer/provider/admin mutations proven), structured pino with runtime-proven never-log redaction | CODE-COMPLETE | P0 | IN-02 | M1 | Engineering (W6) | docs/37 W6-1 record; `test/request-correlation.test.ts`, `test/logging-redaction.test.ts` |
| IN-10 | Observability: metrics, traces, error tracking (all three frontends + API), uptime checks, alerting. VERIFIED: none exists (no otel/sentry/prometheus anywhere; 9 runtime deps) | IMPLEMENTATION-REQUIRED (ADVANCED by W6-4A: CloudWatch baseline is IaC-complete — log groups with bounded retention, metric filters on the W6 alert lines, infrastructure alarms, SNS topic; the human destination/vendor remains an owner input) | P0 | IN-02 | M4 | Engineering (W6) | docs/23 §10.10. Proof: G8 monitoring live with alert routing |
| IN-11 | CI/CD: typecheck/lint/unit/contract/E2E on merge, automated two-phase migrations, one-command deploy with **tested rollback** | IMPLEMENTATION-REQUIRED (ADVANCED by W6-4A: the pipeline exists in the repository — CI (backend on PostgreSQL, three frontends, contract suites, audit policy, production image build + invariants + Trivy) and an OIDC deploy workflow (migrate task → rollout → readiness/smoke; production only by dispatch + environment approval; deploy-DISABLED until the account variables exist); the rehearsed staging deploy + rollback is W6-4B) | P0 | — | M1 | Engineering (W6) | docs/23 §10.9 (today: local scripts only). Proof: rehearsed deploy+rollback in staging |
| IN-12 | Staging environment, config-identical topology, seeded deterministic data (dev seed graduates) | CONFIGURATION-REQUIRED (ADVANCED by W6-4A: the config-identical staging topology is IaC-complete (same root stack, staging var-file, separate account/state); apply and certification are W6-4B) | P0 | IN-01…06 | M1 | Ops | docs/23 §10.8. Proof: staging serving all three frontends |
| IN-13 | Backup/restore rehearsals per service class (financial class first) + region-failure runbook | PRODUCTION-SMOKE-REQUIRED (ADVANCED by W6-4A: the DR outline and restore-rehearsal procedure are documented (`infra/README.md` §9); rehearsals need the deployed instance) | P0 | IN-05 | M4 | Ops | docs/23 §10.11. Proof: rehearsal records |
...
| PA-09 | Reconciliation operations: schedule `reconcileLedgerAgainstProvider` (VERIFIED: pure read, invoked by tests only) + daily ops procedure + `reconciliation_event` persistence when authorized (explicitly deferred, no migration yet) | IMPLEMENTATION-REQUIRED (ADVANCED by W6-3: `payment.reconciliation` runs daily under the scheduler when a provider composes — detection only, ledger proven byte-identical, discrepancy alert with bounded machine facts + all-clear, provider outage isolated with a bounded code. The service's bounded-recent-attempts shape is used UNCHANGED (docs/37 §14's "previous UTC day + 6 h" window would require a service change — recorded, not done). Remaining: the finance/ops procedure, `reconciliation_event` persistence when authorized, and a run against real Stripe TEST truth — PA-04) | P0 | OP-01 | M4 | Engineering + ops | `payment-reconciliation.ts:61-63`; docs/37 W6-3 record; `test/scheduled-jobs.test.ts` |
| PA-10 | Stuck-payment alerting: wire `findStuckPaymentStates` (hooks exist, no alert channel) to real alerting | IMPLEMENTATION-REQUIRED (ADVANCED by W6-3: `payment.stuck-state` every 5 min under the scheduler with the certified thresholds UNCHANGED; per-state alert keys with durable cross-replica dedup (previous run's facts) + re-alert suppression + all-clear when a state vacates; zero mutation proven. Remaining: the real alert DESTINATION — IN-10) | P0 | IN-10, OP-01 | M4 | Engineering | docs/33 W5-6; docs/37 W6-3 record; `test/scheduled-jobs.test.ts` |
...
| ID | Gate | Status | Pri | Depends on | Mile | Owner | Evidence · closure proof |
|---|---|---|---|---|---|---|---|
| OP-01 | **Production scheduler/runner infrastructure.** Originally VERIFIED absent. **CLOSED by W6-3** (OWNER-APPROVED at `d5ef107`): worker-hosted advisory-lock scheduler ticks (`pg_try_advisory_lock` per job on a dedicated session; due-ness on the database clock; re-check under the lock; orphaned runs marked `abandoned`) over migration `0023` `job_run` bookkeeping (identity, start, outcome, bounded error code, items, machine facts, duration, run id — never `audit_event.request_id`); two spawned `start:worker` replicas as `himma_worker` proven (runs never overlap per job, SIGKILL survival, restart, SIGTERM exit 0); crash injection at both boundaries; missed-run + consecutive-failure alerts through the generic seam. Deployed-environment execution is the IN-12/IN-01 smoke, not a code gap | CODE-COMPLETE | P0 | IN-02 | M4 | Engineering (W6) | docs/37 W6-3 record; `test/scheduler.test.ts`, `test/scheduler-process.test.ts`, `test/job-run-migration.test.ts` |
| OP-02 | Payment runner cadence: `sweepLapsedPaidCheckouts` + `processPendingGatewayEvents` + `processTrustedPaymentResults`. Originally VERIFIED webhook-request-only. **CLOSED by W6-3** (OWNER-APPROVED at `d5ef107`): W6-2 supplied the gateway-event + trusted-result cadence; W6-3 adds `payment.checkout-sweep` (60 s, advisory-lock tick calling the certified D-W5-5 sweep unchanged — lapsed holds/purchases wind down, captured/succeeded intents never swept, hold rows/counters untouched, repeat harmless, two replicas → one run). All three run only when a provider composes (absent or TEST — docs/37 §33); the deterministic provider cannot compose in production; D-W5-6 TEST certification with real Stripe TEST credentials stays PA-04 | CODE-COMPLETE | P0 | OP-01 | M4 | Engineering | docs/37 W6-2/W6-3 records; `test/scheduled-jobs.test.ts`, `test/worker-payment.test.ts` |
| OP-03 | Outbox relay + search-projection consumer. **CLOSED by W6-2** (OWNER-APPROVED at `8eb8b08`; implementation `01976e5`): the SKIP LOCKED outbox dispatcher over migration `0022_outbox_delivery_state` (durable claim/ack, server-side backoff, quarantine, unhandled-type diagnosability, crash-injection + two-dispatcher proofs) with the search-projection consumer registered behind it; `npm run start:worker` runs it as `himma_worker`; two spawned production worker processes proven (exactly-once effects, SIGKILL survival, restart resumption, clean SIGTERM) | CODE-COMPLETE | P0 | OP-01 | M4 | Engineering | docs/37 W6-2 record; `test/outbox-dispatcher.test.ts`, `test/search-projection-worker.test.ts`, `test/worker-process.test.ts` |
| OP-04 | Hold-expiry sweep cadence (`sweepExpiredHolds`). Correctness already holds without it — lapsed holds are reclaimed inline on the claim path and ignored by readers; the sweep is maintenance. **CLOSED by W6-3** (OWNER-APPROVED at `d5ef107`): `booking.hold-sweep` every 5 min under the scheduler; the sweep-independence pin (a lapsed hold on a full unit is reclaimed inline with the sweep DISABLED) is proven | CODE-COMPLETE | P1 | OP-01 | M4 | Engineering | docs/37 W6-3 record; `test/scheduled-jobs.test.ts` |
| OP-05 | Identity/staff sweeps (`processExpiredAssignments`, `expireDueStaffInvitations`). **CLOSED by W6-3** (OWNER-APPROVED at `d5ef107`): `identity.role-expiry` + `provider.invitation-expiry` hourly under the scheduler; their audit rows carry `request_id = NULL` (background work); the invitation sweep's parameter type was narrowed to `{ db }` so the worker never holds the invitation pepper (type-only; zero behavior change) | CODE-COMPLETE | P1 | OP-01 | M4 | Engineering | docs/37 W6-3 record; `test/scheduled-jobs.test.ts`, `test/scheduler-process.test.ts` |
| OP-06 | Retention jobs: redemption-attempt window cleanup (recorded "future W6 operational sweep", migration `0018:398`), audit/event retention. NOTE: the app DB role deliberately has no DELETE grant — retention needs a separately provisioned role | IMPLEMENTATION-REQUIRED (ADVANCED by W6-3: the maintenance authority exists — `himma_maintenance` NOLOGIN role + `himma_maintenance_runner` login + bounded SECURITY DEFINER retention functions (migration `0023`) + the `start:maintenance` mode; the ENGINEERING-scoped sweeps are CODE-COMPLETE: rate-limit windows, redemption-lookup windows, completed idempotency keys, published outbox + inbox rows, finished job runs. The POLICY-gated sweeps — `audit_event`, identity/staff security records, verification evidence — have no function and no grant until VE-05/SE-05 rule; nothing is fabricated) | P1 | OP-01, VE-05 | M4 | Engineering | migrations 0001/0002/0004/0018 comments; docs/37 W6-3 record; `test/maintenance-runtime.test.ts` |
| OP-07 | Runner alerting + idempotent-restart certification (every runner already idempotent by design — restart behavior must be proven in the deployed mechanism). **CLOSED by W6-3** (OWNER-APPROVED at `d5ef107`): the generic operational-alert seam (`emitOperationalAlert`-shaped; keyed dedup + re-alert suppression + all-clear; structured-log destination) carries consecutive-failure, missed-run (no success within 3× interval), reconciliation-discrepancy, and stuck-payment alerts; kill/restart proven on real spawned worker AND maintenance processes (SIGKILL mid-run → survivor converges, orphan `abandoned`; SIGTERM → clean exit; re-invocation resumes). The alert DESTINATION (Slack/PagerDuty/email vendor) remains IN-10/OP-08; the drill in the deployed environment is the IN-12 staging smoke | CODE-COMPLETE | P0 | OP-01, IN-10 | M4 | Engineering | docs/37 W6-3 record; `test/scheduler.test.ts`, `test/scheduler-process.test.ts` |
...
| ID | Gate | Status | Pri | Depends on | Mile | Owner | Evidence · closure proof |
|---|---|---|---|---|---|---|---|
| VE-01 | Evidence storage production composition (real S3 driver + bucket; today the only runnable server passes NO evidence storage → fail-closed 404) | CONFIGURATION-REQUIRED (ADVANCED by W6-4A: IaC + task wiring for the real S3 driver composition are complete (bucket, scoped policy on the API task role, `EVIDENCE_S3_*` injection); the smoke needs the staging apply) | P0 | IN-02, IN-06 | M4 | Ops | Verified caller analysis. Proof: upload/head/get smoke against the real bucket |
...
| ID | Gate | Status | Pri | Depends on | Mile | Owner | Evidence · closure proof |
|---|---|---|---|---|---|---|---|
| SE-01 | Production secret inventory + rotation procedure (webhook retiring-secret rotation is CODE-COMPLETE; the procedure and inventory are not) | IMPLEMENTATION-REQUIRED (ADVANCED by W6-4A: the secret inventory/ownership matrix and rotation procedure are documented (`infra/README.md` §6); one rehearsed rotation needs staging) | P0 | IN-04 | M1 | Ops | docs/23 §13. Proof: written inventory; one rehearsed rotation |
...
| SE-03 | Dependency/vulnerability + container scanning in CI (the docs/13 "tooling-only" acceptance EXPIRED at backend start per docs/23 C10) | IMPLEMENTATION-REQUIRED (ADVANCED by W6-4A: the CI gate is defined — `npm audit --audit-level=high` for runtime deps, ECR scan-on-push, Trivy CRITICAL/HIGH on the built image; it becomes active with the first CI run in the company repository) | P0 | IN-11 | M1 | Engineering (W6) | docs/23 §13/C10. Proof: CI gate active with a triage policy |
...
| SE-05 | Audit request-id wiring + audit-log retention (VERIFIED: `audit_event.request_id` never populated; no retention job — recorded W6 scope, not a closed-slice defect) | IMPLEMENTATION-REQUIRED (request-id half closed by W6-1; ADVANCED by W6-3: the retention MECHANISM exists — maintenance authority + bounded function pattern — but `audit_event` retention itself has no function and no grant because its period is a legal/PDPL ruling (OWNER/counsel), and `audit_event` stays immutable by trigger. Nothing is fabricated) | P1 | IN-09, OP-06 | M4 | Engineering | `db/audit.ts`; docs/37 W6-3 record |
...
## 8. W6 runner inventory (verified at LR-0: every job exists as a certified function; NO scheduler of any kind existed)

**W6-3 status note (W6-3 OWNER-APPROVED at `d5ef107`):** the inventory below is the LR-0 snapshot. As of W6-3 every non-destructive runner in it is scheduled by the worker-hosted advisory-lock scheduler (`payment.checkout-sweep` 60 s · `payment.reconciliation` daily · `payment.stuck-state` 5 min · `booking.hold-sweep` 5 min · `identity.role-expiry` hourly · `provider.invitation-expiry` hourly; the W6-2 outbox/search/gateway-event/trusted-result loops continue), and the engineering-scoped retention sweeps run only in the short-lived `maintenance` mode under `himma_maintenance_runner`. Policy-gated retention (audit, identity records, evidence) stays absent pending VE-05/SE-05. Deployment mechanism for all rows remains the open IN-01/IN-11/IN-12 track.
...
|---|---|---|
| **A — Production Platform & Operations (W6)** | IN-02/07/09/10/11/12, OP-01…07, SE-03, VE-01, PA-09/10 wiring | **W6-0 plan [docs/37](37_W6_PRODUCTION_RUNTIME_AND_OPERATIONS_PLAN.md) CLOSED `3c599b8` · W6-1 CLOSED `e01af85` (IN-02/07/09) · W6-2 CLOSED `8eb8b08` (OP-03) · W6-3 CLOSED `d5ef107` (OP-01/02/04/05/07) · infrastructure ruling `0393601` (IN-01) · W6-4A CLOSED `cba2bd8` (AWS foundation, container artifact certified; no closures) · W6-4B (staging deploy/certify) AUTHORIZED — the next gates that can close need real AWS staging evidence ([docs/38](38_PRODUCTION_INFRASTRUCTURE_DECISION.md))** |
