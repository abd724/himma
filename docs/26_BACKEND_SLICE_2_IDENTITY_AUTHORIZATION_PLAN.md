...
**§14.E cookie/CSRF wiring — IMPLEMENTED with the first portal (W2-12A session-continuity correction, 2026-08-17), exactly per §4.7(9):** the Cognito refresh token rides the server-set `himma_refresh` cookie (`Path=/auth; SameSite=Strict; HttpOnly; Secure`, Max-Age configurable via `AUTH_REFRESH_COOKIE_MAX_AGE_SECONDS`, default 30 days); access tokens stay client memory; CSRF is SameSite=Strict PLUS the double-submit `himma_csrf` cookie / `x-csrf-token` header pair (timing-safe, reissued by `GET /auth/csrf` to `PORTAL_ALLOWED_ORIGINS` only), with a config-driven credentialed CORS layer and parse-time refusal of insecure production configuration (`AUTH_COOKIE_SECURE=false`, http origins). `POST /auth/refresh` (the §10 route, now implemented) mediates the provider refresh through the new `ProviderTokenRefresher` adapter port under the §9.4 transaction — `login_session` liveness + CAS last-seen — so Himma revocation refuses a still-valid provider refresh token and clears the channel; rotation rotates the cookie; `POST /auth/logout` clears the channel and uses the cookie material for provider revocation. No refresh token is ever stored in PostgreSQL or echoed in a response body (test-locked); no schema change. The channel registers only when `sessionContinuity` deps are configured — otherwise absent. **The concrete AWS-SDK refresh client and the real-pool REFRESH_TOKEN_AUTH/SOFTWARE_TOKEN_MFA smokes remain §14.E′ operational work — not claimed.**
