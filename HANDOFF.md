...
- **Owner review fixes (post-8a report):** Your week rows restructured — day column (48) then per-session two-line title over a `time · participant` meta line — so long titles ("Reformer Pilates Foundations") render whole at 390 and 360; plan-card titles allow two lines; `dockTokens.contentClearance` raised to `spacing.huge` (40) so every browsing surface's final card rests clear of the dock (formula unchanged: height + safeAreaOffset + safe-area bottom + clearance). QA screenshots: `.__expo_fast_refresh` dev bubble hidden via CSS kill rule in the shot helper; `scrollToHeading` aligns headings to the scroller top (was a no-op when already in viewport, duplicating shots).
- **QA script updates:** `scripts/qa/home-review.mjs` (79 checks: scenario matrix + gate + truncation + dock-clearance assertions, plus a top/mid/bottom `matrix-*` screenshot set for all four scenarios at 390 and 360). `discover-review.mjs`/`results-review.mjs`/`search-review.mjs` Home-era steps updated (no Home chips/hero; search-review now selects Adam on Discover — the old Home-chip step silently no-opped and two search shots are byte-identical to baseline again after the fix).
