...
**Current work: Checkout planning.** The booking milestone is closed and owner-approved (`01b6f73` + `454ee91`). `docs/22_CHECKOUT_PLAN.md` is the current planning document for the Checkout milestone (HMA-021 scope, stopping at a truthful payment-submit contract) — **drafted, awaiting owner approval; Checkout implementation has not started.** It surfaces the VAT/fee/Total, acceptance/consent, payment-method, and submit-behavior decisions (docs/22 §18) that must be resolved before implementation. Payment processing, confirmation, and receipts belong to a later Payment & Confirmation milestone (docs/22 §7.12).
