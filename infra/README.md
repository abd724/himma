...
## 11. Local container harness (EXECUTED — W6-4A container certification, 2026-09-08)

```
infra/local/harness.sh
```
Requires an OCI runtime with Docker-compatible tooling (Docker Engine / Colima + docker CLI, compose v2), openssl, curl. Generates a harness CA + a PostgreSQL server certificate (CN/SAN `postgres`; the network alias `postgres-unlisted` is deliberately NOT in the SAN), random passwords into gitignored `infra/local/.harness.env`, builds the image with the CI build path (`backend/`, `backend/Dockerfile`), brings up TLS PostgreSQL → migrate → verify → provision-roles → api ×2 → worker ×2 with **read-only root filesystems and no tmpfs** (exactly the ECS task definitions), then executes 34 proofs and tears down. Exit 0 = certified; the transcript is the record. Proofs: image facts (non-root uid 10001, Node version, CA readable, forbidden paths absent, all six invocations present) · one-shot migrate/verify/provision exit 0 with evidence lines · migration head + applied count · role model (three LOGIN runtime identities, two NOLOGIN authorities) · every application backend on TLSv1.3 · per-replica identities · readiness/liveness on both API replicas, dev identity 404, forged bearer 401, independent request ids · session configuration from the canonical pool inside the image (login, `statement_timeout` 15s/1min/5min, UTC, TLS) · PID 1 = the application (in-process loader) · read-only root with zero writable locations · worker: no published port, status listener loopback-only · scheduler runs across both workers with zero overlap · `retention.all` as `himma_maintenance_runner` (exit 0, never resident), unknown job → usage exit 2, api/worker credentials refused · DDL/`pgmigrations` denials for every runtime identity · api/worker against an un-migrated database perform no DDL (503 / refused) · `db-migrate` under the api credential refused · two concurrent migration jobs serialize (head once, 0 duplicates), idempotent re-run · TLS from inside the image: verify-full positive, wrong CA refused, hostname mismatch refused, `disable` on a non-loopback host refused, missing mode refused, `?sslmode=` refused · `PAYMENTS_MODE=live`/`deterministic` refused, live-shaped Stripe key refused at startup, `productionChargingPossible === false` through the real entrypoint · SIGTERM → worker and API handlers run, drain, pool closed, exit 0, siblings continue · exposure: only `api` publishes (host loopback, harness-only), one-shots exited.

**First real build (2026-09-08, Colima 0.10.3 / Docker Engine 29.5.2 / Buildx 0.37.0, linux/arm64; CI builds linux/amd64 from the same index digest):** base `node:24-bookworm-slim@sha256:ba849c60be29959425b8734d57b8b4b7d56f98edd9504c9af091d5281095a71e` (Node 24.20.0, Debian 12.15) — pinned in the Dockerfile; RDS global CA bundle sha256 `e5bb2084ccf45087bda1c9bffdea0eb15ee67f0b91646106e466714f9de3c7e3` (108 certificates). Defects found only by execution and fixed: CA directory modes (BuildKit `ADD --chmod` applies to created directories), the tsx CLI entrypoint (child process + writable `/tmp` IPC socket → replaced by the in-process `--import` loader with the compile cache disabled, so the image needs **no writable path**), `dotenv` imported by the migration/verify/provisioning entry points (now lazy, dev-only), a live-shaped Stripe key not refusing startup (now refused in production), Trivy/npm-audit HIGH findings (`fast-uri` bumped in the lockfile; npm/yarn/corepack removed from the runtime stage). The same proofs also run natively in `backend/test/db-tls.test.ts` and the W6-1…W6-3 spawned-process suites; CI builds the image, checks its invariants, and scans it on every push. Note: the process-spawning Jest suites are timing-sensitive — do not run them while the harness is running on the same machine.
